...
(let ()
  ...
  (begin
    (define top-level-eval-hook
      ...
      ; system evaluator
      (lambda (x) (eval `(,noexpand ,x))))

    (define local-eval-hook
      ; for local macro transformers, use interpreter
      (lambda (x) (interpret `(,noexpand ,x))))

    (define define-top-level-value-hook
      $set-top-level-value!)

    (define get-clo-info
      (lambda (sym) ($sgetprop sym '*clo* #f)))

    (define put-clo-info
      (lambda (sym info) ($sputprop sym '*clo* info)))

    (define get-global-definition-hook
      (lambda (sym) ($sgetprop sym '*cte* #f)))
    ...
    (define get-library-descriptor
      (lambda (symbol) ($sgetprop symbol '*library* #f)))

    (define rem-library-descriptor
      (lambda (symbol) ($sremprop symbol '*library*)))
    ...
    (define get-program-descriptor
      (lambda (symbol) ($sgetprop symbol '*program* #f)))

    (define rem-program-descriptor
      (lambda (symbol) ($sremprop symbol '*program*)))

    (define get-global-substs
      (lambda (symbol token) ($sgetprop symbol token #f)))
    ...
    (define generate-id
      (lambda (sym) (gensym (symbol->string sym))))
  ...
  (with-output-language (Lsrc Expr)
    (define ae->src
      (lambda (ae)
        (and (and (annotation? ae)
                  (fxlogtest (annotation-flags ae)
        ...
        (define ae->profile-src
          (lambda (ae)
            (and (and (annotation? ae)
                 ...
                 (annotation-source ae))))
        (if (and (eq? ($compile-profile) 'source) (generate-profile-forms))
    ...
    (module (build-lambda build-library-case-lambda build-case-lambda)
      ...
      (define build-clauses
        (lambda (clauses)
          (map (lambda (x) (build-clause (car x) (cadr x)))
               clauses)))
      ...
      (define build-case-lambda
        (lambda (ae clauses)
          (build-profile ae
            `(case-lambda
               ,(make-preinfo-lambda (ae->src ae) #f)
        ...
        (lambda (ae libspec clauses)
          (build-profile ae
            (let ([clauses (build-clauses clauses)])
              ...
              `(case-lambda
                 ,(make-preinfo-lambda (ae->src ae) #f libspec)
                 ,clauses
                 ...))))))
    ...
    (define build-primitive-reference
      (lambda (ae name)
        (if ($suppress-primitive-inlining)
            ...
            (build-profile ae
              (lookup-primref (fxmax (optimize-level) 2) name)))))
    ...
    (module (build-global-reference build-global-assignment)
      ...
      (define build-global-reference
        (lambda (ae name safe?)
          (when (eq? (subset-mode) 'system)
                (unbound-warning (ae->src ae) "reference to" name))
          (build-primcall ae
            (if (or safe? (fx= (optimize-level) 3)) 3 2)
            '$top-level-value
            `(quote ,name))))
      ...
      (define build-global-assignment
        (lambda (ae name val)
          (when (eq? (subset-mode) 'system)
                (unbound-warning (ae->src ae) "assignment to" name))
          (build-primcall ae 3 '$set-top-level-value! `(quote ,name) val))))
    ...
    (define build-visit-only
      (lambda (exp)
        (with-output-language (Lexpand Outer) `(visit-only ,exp))))

    (define build-revisit-only
      (lambda (exp)
        (with-output-language (Lexpand Outer) `(revisit-only ,exp))))
    ...
    (define build-moi
      (let ([moi-record `(moi)]) (lambda () moi-record)))
    ...
    (define build-pariah
      (lambda (ae e) (build-profile ae `(seq (pariah) ,e))))
    ...
    (define build-data
      (lambda (ae exp) (build-profile ae `(quote ,exp))))
                  ...
                  (if (null? types)
                      ...
                      (let ([var (car vars)] [val-exp (car val-exps)])
                        (let-values ([(vars val-exps)
                                      (f (cdr types) (cdr vars) (cdr val-exps))])
                          (if (eq? (car types) 'global)
                              (values (cons (build-lexical-var no-source 'ignore)
                                            vars)
                                      (cons (build-global-assignment no-source var val-exp)
                                            val-exps))
                              (values (cons var vars) (cons val-exp val-exps)))))))])
              ...
              (build-letrec
                ...
                (if (null? sets)
                    ...
                    (build-sequence no-source
                      (append sets (list body-exp)))))))))

    (define build-top-program
      (lambda (uid body-exp)
        (with-output-language (Lexpand Program) `(program ,uid ,body-exp))))

    (define build-recompile-info
      (lambda (import-req* include-req*)
        (make-recompile-info
          (remp (lambda (x)
                  (libdesc-system? (get-library-descriptor (libreq-uid x))))
                import-req*)
          include-req*)))
    ...
    (define build-lexical-var
      (lambda (ae id) (make-prelex id 0 ae #f)))

    (define lexical-var-assigned?
      (lambda (x) (prelex-assigned x)))

    (define build-input-profile
      (lambda (src) `(profile ,src))))

  (define unannotate
    (lambda (e)
      (if (annotation? e) (annotation-expression e) e)))
  ...
  (define make-rho (lambda () (make-eq-hashtable)))
  ...
  (define retract-rho!
    (lambda (r label) (eq-hashtable-delete! r label)))

  (define lookup-rho
    (lambda (r label) (eq-hashtable-ref r label #f)))
  ...
  (define out-of-phase-binding
    (make-binding 'displaced-lexical "out-of-phase identifier"))
  ...
  (define displaced-lexical-error
    (lambda (id what why)
      (cond
        [(string? why)
         (syntax-error id (format "attempt to ~a ~a" what why))]
        [(id->label id empty-wrap)
         (syntax-error id
           (format "attempt to ~a out-of-context identifier" what))]
        [else
         ($undefined-violation id
           (format "attempt to ~a unbound identifier" what))])))
    ...
    (lambda (who x)
      (cond
        ...
        [(variable-transformer? x)
         (make-binding 'macro! (variable-transformer-procedure x))]
  ...
  (define id-sym-name&marks
    (lambda (x w)
      (if (syntax-object? x)
          (values (unannotate (syntax-object-expression x))
                  (join-marks (wrap-marks w)
  ...
  (define-syntax top-marked?
    (syntax-rules () ((_ w) (memq 'top (wrap-marks w)))))

  (define-syntax only-top-marked?
    (syntax-rules ()
      ((_ id)
       (same-marks? (wrap-marks (syntax-object-wrap id))
         (wrap-marks top-wrap)))))
  ...
  (define make-label/pl
    (case-lambda
      [(label pl) (if (null? pl) label (cons label pl))]
      [(label p pl)
       (cons* label p (remp (lambda (q) (eq? (car q) (car p))) pl))]))

  (define label/pl->label
    (lambda (label/pl)
      (if (pair? label/pl) (car label/pl) label/pl)))
  ...
  (define get-indirect-interface
    ...
    (lambda (x) (if (symbol? x) ($top-level-value x) x)))
  ...
  (define-syntax make-empty-ribcage
    (syntax-rules () ((_) (make-extensible-ribcage '()))))
      ...
      (let ([sym (unannotate (syntax-object-expression id))]
        ...
        (let ([ht (if (and (pair? chunks) (symbol-hashtable? (car chunks)))
          ...
          (let ([a (symbol-hashtable-cell ht sym '())])
            (set-cdr! a
              (cons (cons (wrap-marks (syntax-object-wrap id)) label/pl)
                    (cdr a))))))))
        ...
        (or (and new
                 (let f ([new new])
                   (cond
                     ...
                     [(symbol? new)
                      (and (same-marks? marks (wrap-marks top-wrap))
                           new)]
                     [(same-marks? marks
                        (wrap-marks (syntax-object-wrap new)))
                      (resolved-id->label/pl new)]
                     [else #f])))
            (and (eq? token '*system*)
                 (same-marks? marks (wrap-marks top-wrap))
                 (or ($sgetprop sym '*flags* #f)
  ...
  (define store-global-subst
    (lambda (id token new-marks)
      (define cons-id
        (lambda (id x) (if (not x) id (cons id x))))
      (define weed
        (lambda (marks x)
          (if (pair? x)
              ...
              (and x
                   (not (same-marks? (id-marks x) marks))
                   x))))
              ...
              (let ([marks (id-marks id)])
                ...
                (let ([x (weed marks old-substs)])
                  (if (and (same-marks? marks (wrap-marks top-wrap))
                           (or (symbol? id)
                               (null? (resolved-id->pl id))))
                      ; need full id only if more than top-marked
                      (let ([label (if (symbol? id)
                                       id
  ...
  (define make-resolved-id
    (lambda (sym marks label/pl)
      ...
      (make-syntax-object sym
        (make-wrap marks
          (list (make-fixed-ribcage (vector sym)
                  (vector marks)
                  (vector label/pl)))))))
  ...
  (define resolved-id->label/pl
    (lambda (id)
      (vector-ref
        (fixed-ribcage-label/pls
          (car (wrap-subst (syntax-object-wrap id))))
        0)))

  (define resolved-id->label
    (lambda (id)
      (label/pl->label (resolved-id->label/pl id))))

  (define resolved-id->pl
    (lambda (id) (label/pl->pl (resolved-id->label/pl id))))

  ;;; Scheme's append should not copy the first argument if the second is
  ;;; empty, but it does, so we define a smart version here.
  (define smart-append
    (lambda (m1 m2) (if (null? m2) m1 (append m1 m2))))
    ...
    (lambda (w1 w2)
      (let ((m1 (wrap-marks w1)) (s1 (wrap-subst w1)))
        (if (null? m1)
            (if (null? s1)
                w2
                (make-wrap (wrap-marks w2) (join-subst s1 (wrap-subst w2))))
            (make-wrap (join-marks m1 (wrap-marks w2))
              (join-subst s1 (wrap-subst w2)))))))

  (define join-marks (lambda (m1 m2) (smart-append m1 m2)))

  (define join-subst (lambda (s1 s2) (smart-append s1 s2)))
      ...
      [(_ k?)
       (let ()
         (define-syntax return
           (if k?
               (syntax-rules ()
                 [(_ label retry) (values label retry)])
             ...
             (if (null? subst)
                 ...
                 (let ([fst (car subst)])
                   (cond
                     ...
                     [(fixed-ribcage? fst)
                      (search-fixed-ribcage sym subst marks fst)]
                     [(extensible-ribcage? fst)
                      (search-extensible-ribcage sym subst marks fst)]
                     [(top-ribcage? fst)
                      (cond
                 ...
                 (let f ((i 0))
                   (cond
                     ...
                     ((and (eq? (vector-ref symnames i) sym)
                           (same-marks? marks
                             (vector-ref (fixed-ribcage-marks ribcage) i)))
                        ...
                        (let g ([ls (symbol-hashtable-ref chunk sym '())])
                          (if (null? ls)
                              ...
                              (if (same-marks? marks (caar ls))
                                  (return (cdar ls) (lambda () (f (cdr chunks))))
                                  (g (cdr ls)))))]
                       [(import-interface? chunk)
                        (cond
                          [(iface-id->label/pl
                           ...
                           =>
                           (lambda (label/pl)
                             (return label/pl (lambda () (f (cdr chunks)))))]
                          [else (f (cdr chunks))])]
                       [(barrier? chunk)
  ...
  (define id->label
    (lambda (id w) (label/pl->label (id->label/pl id w))))
    ...
    (lambda (i j)
      (let ([x (id->label i empty-wrap)])
        (and (eq? x (id->label j empty-wrap))
             (if (eq? x #f)
                 (eq? (id-sym-name i) (id-sym-name j))
                 #t)))))
  ...
  (define valid-bound-ids?
    (lambda (ids)
      (and (let all-ids? ((ids ids))
             (or (null? ids)
                 (and (id? (car ids)) (all-ids? (cdr ids)))))
  ...
  (define symbolic-id=?
    (lambda (id sym) (eq? (id-sym-name id) sym)))

  (define-syntax sym-kwd?
    (syntax-rules ()
      [(_ id kwd ...) (and (id? #'id) (or (symbolic-id=? #'id 'kwd) ...))]))
    ...
    (lambda (when-list w)
      ...
      (map (lambda (x)
             (cond
               ...
               [else
                (syntax-error (wrap x w) "invalid eval-when situation")]))
      ...
      (cond
           ...
           (if (id? first)
               (let* ([b (lookup (id->label first w) r)]
                 ...
                 (case type
                   ...
                   [(define-property)
                    (values 'define-property-form #f e w ae)]
                   ...
                   [(indirect-export)
                    (values 'indirect-export-form #f e w ae)]
                   [(implicit-exports)
                    (values 'implicit-exports-form #f e w ae)]
        ...
        [(syntax-object? e)
         (syntax-type
           ...
           #f
           rib)]
        [(annotation? e) (syntax-type (annotation-expression e) r w e rib)]
        [(symbol? e)
      ...
      (fluid-let ([require-import (library-collector #f)]
        ...
        (let ([ribcage (make-empty-ribcage)])
          (let ([w (make-wrap (wrap-marks w) (cons ribcage (wrap-subst w)))])
            (let ([code* (chi-top (list (make-frob (wrap e w) #f))
                                  (make-rho)
                                  ctem
                                  rtem
                                  ribcage
                    ...
                    (case type
                           ...
                           (let ([sym (id-sym-name id)])
                             (if (and (eq? (subset-mode) 'system)
                                 ...
                                 (begin
                                   ...
                                   (parse (cdr frob*)
                                     (cons (bodit-system-define sym (wrap rhs w) ae)
                                           bf*)
                                     #f
                                     label*))
                                 (let-values ([(label bound-id)
                                               (let ([marks (wrap-marks (syntax-object-wrap id))])
                                                 (cond
                                                   [(and (not (equal? marks (wrap-marks top-wrap)))
                                                    ...
                                                    =>
                                                    (lambda (label)
                                                      (values label (make-resolved-id sym marks label)))]
                                                   [else
                                   ...
                                   (if meta?
                                       ...
                                       (let ([b (make-binding 'global label)])
                                         (extend-rho! r label b (fxlognot 0))
                                         (parse (cdr frob*)
                                           (cons (bodit-define bound-id label b (wrap rhs w) ae)
                                                 bf*)
                                           #f
                                           label*))))))))]
                      [(define-syntax-form)
                             ...
                             (if (null? impspec*)
                                 ...
                                 (fluid-let ([require-import
                                   ...
                                   (let-values ([(mid tid imps)
                                                 (determine-imports (car impspec*) r std?)])
                                     (let ([bf* (process-impspecs (cdr impspec*))])
                                       (if (import-interface? imps)
                                           ...
                                           (for-each
                                             ...
                                             imps))
                                       (cons (bodit-import mid imps (require-import))
                                             bf*)))))))
                         (parse (cdr frob*)
                           (process-impspecs impspec*)
                           #f
                           label*))]
                      [(export-form)
                       (parse-export e w ae)
                      ...
                      [(alias-form)
                       (let-values ([(new-id old-id) (parse-alias e w ae)])
                         (let ([new-id (wrap new-id w)]
                           ...
                           (when (displaced-lexical? new-id r)
                           ...
                           (parse (cdr frob*)
                             (let ([id (make-resolved-id
                                         (id-sym-name new-id)
                                         (wrap-marks (syntax-object-wrap new-id))
                                         label/pl)])
                               (cons (bodit-alias id) bf*))
                             #f
                             label*)))]
                      [(begin-form)
                       (parse (append (map (lambda (e)
                                             (make-frob (wrap e w) meta?))
                                           (parse-begin e w ae #t))
                                      (cdr frob*))
                              bf*
                              #f
                              label*)]
                      ...
                      [(local-syntax-form)
                       (fluid-let ([require-invoke (library-collector #t)]
                                   [require-visit (library-collector #f)])
                         (let-values ([(forms w ae new-label*)
                                       (chi-local-syntax value #t e r w ae)])
                               ...
                               (let-values ([(label bound-id)
                                 ...
                                 (let ([iface (make-interface
                                   ...
                                   (let ([b (make-binding '$module iface)])
                                     (extend-rho! r label b (fxlognot 0))
                                     (parse (cdr frob*)
                                       (cons* (bodit-module bound-id iface)
                                         (bodit-code
                                           (build-group
                                             `(,@(map (build-requirement '$import-library)
                                                      (require-import))
                                               ,@(map (build-requirement '$invoke-library)
                                                      (require-invoke))
                                               ,@(map (build-requirement '$visit-library)
                                                      (require-visit))
                                               ,@code*)))
                                         bf*)
                                       #f
        ...
        (let process-forms ([bf* bf*] [rcode* '()])
          (if (null? bf*)
              (begin
                ...
                ; just local-syntax labels, which are all local
                (reverse rcode*))
              (bodit-case (car bf*)
                [define (id label binding rhs ae)
                ...
                [system-define (label rhs ae)
                  (process-forms (cdr bf*)
                    (cons (rt-eval/residualize rtem
                            (lambda ()
                              (fluid-let ([require-invoke (library-collector #f)]
                                          [require-visit (library-collector #f)])
                                (residualize-invoke-requirements
                                  (let ([rhs (not-at-top (chi rhs r empty-wrap))])
                                    (if (eq? (binding-type (lookup-global label))
                                             'primitive)
                            ...
                            (define do-top-import
                              (lambda (mid binding)
                                (let ([top-token (top-ribcage-key top-ribcage)])
                                  (ct-eval/residualize ctem
                                    (lambda ()
                                      ($sc-put-cte mid binding top-token))
                                    (lambda ()
  ...
  (module (libdesc-import-req*
            ...
            libdesc-loaded-import-reqs-set!
            libdesc-loaded-visit-reqs
            libdesc-loaded-visit-reqs-set!
            libdesc-import-code
            libdesc-import-code-set!
            libdesc-visit-code
            libdesc-visit-code-set!
            libdesc-visit-id*
            libdesc-visit-id*-set!
            libdesc-clo*
            libdesc-clo*-set!)
    (define get-ctdesc
      (lambda (desc)
    ...
    (define libdesc-visit-req*
      (lambda (desc) (ctdesc-visit-req* (get-ctdesc desc))))
  ...
  (module (libdesc-invoke-req*
    ...
    (define libdesc-invoke-code
      (lambda (desc) (rtdesc-invoke-code (get-rtdesc desc))))
             ...
             [(libdesc-visit-code desc)
              ...
              (lambda (p)
                ...
                (on-reset (begin
                            (for-each
                              (lambda (id)
                                ($sc-put-cte id (make-binding 'visit uid) #f))
                              (libdesc-visit-id* desc))
                            (libdesc-visit-code-set! desc p))
                          (for-each
                            (lambda (req)
                              (visit-library (libreq-uid req)))
                            (libdesc-visit-visit-req* desc))
                          (for-each
                            (lambda (req)
                              (invoke-library (libreq-uid req)))
  ...
  (define-threaded require-invoke
    (lambda (uid)
      ($oops 'sc-expand-internal
             "no one is collecting invoke requirements")))
                ...
                (unless (memp (lambda (x) (eq? (libreq-uid x) uid)) req*)
                        (set! req*
                          (cons (make-libreq (libdesc-path desc)
                                ...
                                req*))))]
             [else
              ($oops #f "library ~:s is not defined" uid)])]
          [() req*]))))
    ...
    (lambda (orig library-path
      ...
      (fluid-let ([require-import (library-collector #f)]
        ...
        (let-values ([(mb* inits exports iface-vector chexports label*)
          ...
          (let ([visit-visit-req* (require-visit)]
                [visit-req* (require-invoke)])
            ; dl*: define label (#f for locals), dv* & de*: define var & expr
            (let process-bindings ([mb* mb*]
                                   [env* '()]
                                   [vthunk void]
                                   [vcode* '()]
                                   [dl* '()]
                        ...
                        (let ([env* (map (lambda (x)
                                           ...
                                           (let ([label (car x)])
                                             (if (and (eq? (binding-type (cdr x)) 'library-global)
                                                      (lexical-var-assigned?
                                                        (binding-value (lookup label r))))
                                                 `(,label . ,unexported-assigned-binding)
                                                 x)))
                                         env*)]
                              [db* (map (lambda (dl) (and dl (box '())))
                                        dl*)]
                              [interface-binding
                                (make-binding '$module
                                  (make-interface
                                    (wrap-marks (syntax-object-wrap template-id))
                                    iface-vector))])
                          ...
                          (for-each
                            (lambda (x)
                              ($sc-put-cte (car x) (cdr x) #f))
                            env*)
                          (for-each
                            (lambda (dl db)
                              (when dl (put-clo-info dl db)))
                            dl*
                            db*)
                  ...
                  (let ([mb (car mb*)] [mb* (cdr mb*)])
                    (mbodit-case mb
                      ...
                      [define-syntax (id label binding rhs)
                        (if (mbodit-exported mb)
                            (process-bindings mb*
                              (cons `(,label . ,(make-binding 'visit library-uid))
                                    env*)
                              (lambda ()
                                ($sc-put-cte label binding #f)
                                (vthunk))
                              (cons (build-checking-cte-install label rhs #f)
                                    vcode*)
                              dl*
                              dv*
                              de*)
                        ...
                        (if (mbodit-exported mb)
                            (process-bindings mb*
                              (cons `(,(cdr association)
                                      .
                                      ,(make-binding 'visit library-uid))
                                    env*)
                              (lambda ()
                      ...
                      [module (id label iface module-mb*)
                        (let ([mb* (append module-mb* mb*)])
                          (if (mbodit-exported mb)
                              (process-bindings mb*
                                (cons `(,label . ,(make-binding '$module iface))
                                      env*)
                                vthunk
                                vcode*
                                dl*
                        ...
                        (if (mbodit-exported mb)
                            ...
                            (process-bindings mb*
                              (cons `(,label . ,unexported-binding)
                                    env*)
                              vthunk
                              (cons (build-global-assignment no-source label expr)
                                    vcode*)
                              dl*
                              dv*
                              de*))]
                      ...
                      [interleaved-init (frob)
                        ($oops 'sc-expand-internal
                               "unexpected interleaved init mbodit")])))))))))
      ...
      (let-values ([(mb* inits exports iface-vector chexports label*)
        ...
        (let process-bindings ([mb* mb*]
          ...
          (if (null? mb*)
              ...
              (values (let ([de* (not-at-top (chi-frobs de* r))]
                            [inits (not-at-top (chi-frobs inits r))])
                        ; verify exports are defined only after processing rhs, init, and
                        ; body expressions so we get syntax, invalid define context, and
                        ; other errors that might explain why exports are actually missing
  ...
  (define id-set-diff
    (lambda (exports defs)
      (cond
        ((null? exports) '())
        ((bound-id-member? (car exports) defs)
  ...
  (module (make-defn-table
    ...
    (define-record-type defn-table
      ...
      (protocol (lambda (new)
                  (lambda ()
                    (new (make-hashtable symbol-hash eq?) '())))))
    ...
    (define record-iface!
      (lambda (tbl import-iface)
        (let ([iface (get-indirect-interface
                       (import-interface-interface import-iface))]
              [new-marks (import-interface-new-marks import-iface)])
          (vector-for-each
            (lambda (id)
              (check-and-record! tbl
                (id-sym-name id)
  ...
  (define-record-type iexports-list
    (fields (mutable id*))
    ; #f if already been or being processed
    (nongenerative)
    (sealed #t))
                         ...
                         [meta? (let ([b (make-binding 'meta-variable label)])
                                  ...
                                  (let ([exp (not-at-top (meta-chi rhs r w))])
                                    ...
                                    (parse (cdr body)
                                      ...
                                      (cons label label*))))]
                         [else
                          (let ([b (make-binding 'dunno
                                     (make-frob (wrap rhs w) meta?))])
                            (extend-rho! r label b 0)
                            (parse (cdr body)
                              (cons (mbodit-define #f #f id label b)
                                    mb*)
                              inits
                              chexports
                              #f
                   ...
                   (let-values ([(id rhs w) (parse-define-syntax e w ae)])
                     (let* ([id (wrap id w)]
                       ...
                       (let ([b (defer-or-eval-transformer
                         ...
                         (extend-rho! r label b (fxlognot 0))
                         (parse (cdr body)
                           (cons (mbodit-define-syntax #t #f id label b exp)
                                 mb*)
                           inits
                           chexports
                           #f
                       ...
                       (let-values ([(*mb* *inits
                         ...
                         (let ([iface (make-interface
                           ...
                           (let ([b (make-binding '$module iface)])
                             (extend-rho! r label b (fxlognot 0))
                             (parse (cdr body)
                               (cons (mbodit-module #f #f id label iface *mb*)
                                     mb*)
                               inits
                               (lambda ()
                                 (chexports)
                   ...
                   (let ([e (source-wrap e w ae)])
                     ...
                     (if (eq? what 'program)
                         (parse (cdr body)
                           ...
                           impind?
                           label*)
                         (let f ([body (cons (make-frob e meta?)
                                             (cdr body))] [mb* mb*])
                           (if (or (null? body)
                                   (not (frob-meta? (car body))))
                               (return mb*
                                 (append inits body)
                                 chexports
                                 expspec**
                               ...
                               (let ([x (not-at-top (meta-chi-frob (car body) r))])
                                 ...
                                 (f (cdr body)
                                    (cons (mbodit-meta-eval #t #f x)
                                          mb*)))))))])))))))

  (define update-mode-set
    (let ([table '((L (load . L) (compile . C) (visit . V) (revisit . R))
                   (C (eval . C))
    ...
    (case-lambda
      ...
      [(rtem eval-thunk residualize-thunk)
       (if (memq 'E rtem)
           ...
           (begin
             (when (memq 'C rtem)
                   (top-level-eval-hook (eval-thunk)))
             (if (memq 'V rtem)
  ...
  (define residualize-import-requirements
    (lambda (import* code)
      (build-sequence no-source
        `(,@(map (build-requirement '$import-library) import*) ,code))))
  ...
  (define meta-chi-frob
    (lambda (x r) (meta-chi (frob-e x) r empty-wrap)))

  (define meta-chi
    (lambda (e r w)
      (parameterize ([meta-level (fx+ (meta-level) 1)]) (chi e r w))))
    ...
    (lambda (type value e r w ae)
      (case type
        ...
        ((call)
         (chi-application
           (let-values ([(type value e w ae)
                         (syntax-type (car e) r w no-source #f)])
             (case type
               [(library-global)
        ...
        ((library-global)
         ...
         (build-global-reference ae (cdr value) #t))
        ((local-syntax-form)
         (let-values ([(forms w ae label*)
                       (chi-local-syntax value #f e r w ae)])
                        ...
                        (case-lambda
                          [(id)
                           ...
                           (let ([b (lookup (id->label id empty-wrap) r)])
                             (case (binding-type b)
                               [(ctv)
                                ($compile-time-value-value (binding-value b))]
                               [else #f]))]
                          [(id key-id)
                           (unless (identifier? id)
                                   (syntax-error
                                    ...
                                    (lambda (a)
                                      (let ([b (lookup* (cdr a) r)])
                                        (case (binding-type b)
                                          ...
                                          [else #f])))]
                                   [else
                                    (let-values ([(new-id-label/pl retry) (retry)])
                                      (and new-id-label/pl
                                           (eq? (label/pl->label new-id-label/pl)
                                                (label/pl->label id-label/pl))
                ...
                (case type
                  [(define-form)
                   (let-values ([(id rhs w ae) (parse-define e w ae)])
                     (let ((id (wrap id w)))
                       (cond
                         [meta? (let ([sym (generate-id (id-sym-name id))])
                                  (let ([label (make-local-label
                                                 (make-binding 'meta-variable sym)
                                                 (fxlognot (fx+ (meta-level) 1)))])
                                    (record-id! defn-table id label)
                                    (extend-ribcage! ribcage id label)
                        ...
                        [(*body *defn-table
                         ...
                         (chi-internal
                           *ribcage
                           ...
                           #t
                           label*)]
                        [(exports exports-to-check iface-vector)
                         (determine-exports 'module orig *expspec** r)])
    ...
    (lambda (e w ae body-wrap)
      (syntax-case e ()
        [(_ orig (dir ... file) library-version uid form ...)
         (values #'orig
           (datum (dir ... file))
           (datum library-version)
           (strip #'uid w)
           (wrap #'file w)
           (map (lambda (x) (wrap x body-wrap))
                #'(form ...)))]
        [_ (syntax-error (source-wrap e w ae))])))
      ...
      (define vmap
        (lambda (fn v)
          (do ((i (fx- (vector-length v) 1) (fx- i 1))
              ...
              ((fx< i 0) ls))))
      (let ([iface (get-indirect-interface
                     (import-interface-interface imp-iface))]
          ...
          (syntax-case x ()
            ...
            [(?prefix *x prefix-id)
             (if std?
                 ...
                 (sym-kwd? ?prefix prefix add-prefix))
             (let ()
               (define prefix-add
                 (lambda (prefix id)
                   (make-resolved-id
                   ...
                   (let ([s (symbol->string (id-sym-name id))])
                     (let ([np (string-length prefix)]
                       ...
                       (unless (and (>= ns np)
                               ...
                               (syntax-error x
                                 (format "missing expected prefix on ~s"
                                         (id-sym-name id))))
                       (make-resolved-id
      ...
      (lambda (what src expspec** r)
          ...
          (lambda (old new id*)
            (cond
              [(id->label/pl old empty-wrap)
               =>
               (lambda (label/pl)
                 (add-id (make-resolved-id (id-sym-name new)
                           (id-marks new)
                           label/pl)
                         id*))]
              ...
              (let ([a (symbol-hashtable-cell ht (id-sym-name id) '())])
                (cond
                  [(find (lambda (x) (bound-id=? id x)) (cdr a))
                   =>
                   (lambda (x)
                     (if (equal? (resolved-id->label/pl id)
                                 (resolved-id->label/pl x))
                         id*
                         (syntax-error src
                           (format "attempt to export multiple bindings for ~s from ~a"
                                   (id-sym-name id)
                                   what))))]
                  [else
                   (set-cdr! a (cons id (cdr a)))
                   (cons id id*)])))))
        ...
        (let g ([expspec** expspec**]
          ...
          (if (null? expspec**)
              ...
              (let-values
                ([(exports exports-to-check new-exports)
                            ...
                            (syntax-case x ()
                              ...
                              [(?import impspec ...)
                               ...
                               (let process-impspecs ([impspec* #'(impspec ...)])
                                 (if (null? impspec*)
                                     (values exports exports-to-check new-exports)
                                     (let-values ([(_mid _tid imps)
                                                   (help-determine-imports (car impspec*) r #f)]
                                                  [(exports exports-to-check new-exports)
                                                   (process-impspecs (cdr impspec*))])
                                       (let ([imps (if (import-interface? imps)
                                                       (module-exports imps)
                                                       imps)])
                                         (values (append (map car imps) exports)
                                                 exports-to-check
                                                 (fold-right
                                                   add-id
                                                   new-exports
               ...
               (let ([new-vars (map gen-var old-ids)])
                 (let ([labels (map make-lexical-label new-vars)])
                   (let ([body (chi-body (syntax (e1 e2 ...))
                     ...
                     (map kill-local-label! labels)
                     (values (let f ((ls1 (cdr new-vars)) (ls2 (car new-vars)))
                               (if (null? ls1)
                                   ls2
    ...
    (lambda (vars)
      (let lvl ((vars vars) (ls '()) (w empty-wrap))
        (cond
          ...
          ((annotation? vars) (lvl (annotation-expression vars) ls w))
  ...
  (define-threaded require-import
    (lambda (uid)
      ($oops 'sc-expand-internal
             "no one is collecting import requirements")))
  ...
  (module (install-library
    ...
    (module (search-loaded-libraries
      ...
      (module (make-root insert-path delete-path search-path list-paths)
        ...
        (define-record-type file
          ...
          (sealed #t))
        (define make-root
          (lambda () (make-dir "root" '() '())))
        (define insert-path
          (lambda (root path uid)
            (let f ([dot root] [path path])
              (cond
                [(null? (cdr path))
                 (make-dir (dir-name dot)
                   (dir-dir* dot)
                   (cons (make-file (car path) uid)
                         (remp (lambda (x) (eq? (file-name x) (car path)))
                               (dir-file* dot))))]
                [(find (lambda (x) (eq? (dir-name x) (car path)))
                       (dir-dir* dot))
                 =>
                 (lambda (dir)
                   (make-dir (dir-name dot)
                     (cons (f dir (cdr path))
                           (remq dir (dir-dir* dot)))
                     (dir-file* dot)))]
                [else
                 (make-dir (dir-name dot)
                   (cons (f (make-dir (car path) '() '()) (cdr path))
                         (dir-dir* dot))
                   (dir-file* dot))]))))
        (define delete-path
          (lambda (root path)
          ...
          (lambda (root path)
            (let loop ([dot root] [path path])
              (cond
                [(null? (cdr path))
                 (cond
                   [(find (lambda (x) (eq? (file-name x) (car path)))
                          (dir-file* dot)) => file-lib]
                   [else #f])]
                [(find (lambda (x) (eq? (dir-name x) (car path)))
                       (dir-dir* dot))
                 =>
    ...
    (define uninstall-library
      (lambda (path uid)
        (with-tc-mutex (rem-library-descriptor uid)
      ...
      (lambda (name)
        (syntax-case name ()
          ...
          [(dir-id ... file-id (version ...))
           (andmap (lambda (x) (and (integer? x) (exact? x) (>= x 0)))
                   (datum (version ...)))
           (let ([uid (generate-id (datum file-id))])
             (values #'(dir-id ... file-id)
                     #'(version ...)
                     (datum->syntax #'* uid)))]
        ...
        (let-values ([(src-path obj-path obj-exists?)
          ...
          (unless (or (not src-path) (string? src-path))
                  ($oops 'library-search-handler
                         "returned invalid source-file path ~s"
                         src-path))
          (unless (or (not obj-path) (string? obj-path))
                  ($oops 'library-search-handler
                         "returned invalid object-file path ~s"
                         obj-path))
      ...
      (lambda (who path dir* all-ext*)
        (define-syntax with-message
          (syntax-rules ()
            [(_ msg e1 e2 ...)
             (begin
               (when (import-notify)
                     (fprintf (console-output-port) "~s: ~a\n" who msg))
               e1
               e2
               ...)]))
        (define make-path
        ...
        (let ([rpath (format "~a~{/~a~}" (car path) (cdr path))])
          (let dloop ([dir* (if (path-absolute? rpath)
                                (with-message
                                  (format "ignoring library-directories since ~s is absolute"
                                          rpath)
                                  '(("" . "")))
                                dir*)])
            (if (null? dir*)
                                ...
                                (if (member (cdr ext) tried)
                                    ...
                                    (let ([obj-path (make-path (cdr dir) rpath (cdr ext))])
                                      (if (file-exists? obj-path)
                                          (with-message
                                            (format "found object file ~s, continuing search for corresponding source file"
                                                    obj-path)
                                            (values ; found object file...now see if we find source file in a source
                                              ; directory that's paired with the same object directory
                                              (let second-chance-dloop ([dir* (cdr dir*)])
                                                (if (null? dir*)
                                                    (with-message
                                                      (format "did not find corresponding source file")
                                                      #f)
                                                              ...
                                                              (if (string=? (cdar ext*) (cdr ext))
                                                                  (let ([src-path (make-path (caar dir*) rpath (caar ext*))])
                                                                    (if (file-exists? src-path)
                                                                        (with-message
                                                                          (format "found corresponding source file ~s"
                                                                                  src-path)
                                                                          src-path)
                                              ...
                                              #t))
                                          (with-message
                                            (format "did not find object file ~s" obj-path)
                                            (obj-loop (cdr ext*) (cons (cdr ext)
                                                             tried)))))))))
                        (let ([ext (car ext*)])
      ...
      (lambda (path version-ref
        ...
        (define-syntax with-message
          (syntax-rules ()
            [(_ msg e1 e2 ...)
             (begin
               (when (import-notify)
                     (fprintf (console-output-port) "import: ~a\n" msg))
               e1
               e2
               ...)]))
        (define verify-uid
        ...
        (define do-load/reload/recompile-library
          (lambda (src-path obj-path compile-file?)
            (let ([found-uid
                    (guard (c [(and ($recompile-condition? c)
                                    (eq? ($recompile-importer-path c) path))
                               (with-message
                                 (format "re~:[loading~;compiling~] ~s because a dependency has changed"
                                         compile-file?
                                         src-path)
                                 (parameterize
                                   ([source-directories
                      ...
                      (parameterize
                        ([source-directories
                           (cons (path-parent src-path)
                                 (source-directories))])
                        (guard (c [(and (irritants-condition? c)
                                        (member obj-path (condition-irritants c)))
                                   (with-message
                                     (with-output-to-string
                                       (lambda ()
                                         ...
                                         (display-condition c)))
                                     ($oops/c #f
                                       ($make-recompile-condition path)
                                       "problem loading object file ~a ~s"
                                       obj-path
                                       c))])
                               ($load-library obj-path (if ct?
                                                           'load
                                                           'revisit))))
                      (cond
                        [(search-loaded-libraries path)
                                   ...
                                   (let ([obj-time (file-modification-time obj-path)])
                                     (for-each
                                       (lambda (include-req)
                                         ((guard (c [else
                                                     (with-message
                                                       (format "missing include file ~a" include-req)
                                                       ($oops/c #f
                                                         ($make-recompile-condition path)
                                                ...
                                                (let ([t (file-modification-time include-req)])
                                                  (lambda ()
                                                    (when (time>? t obj-time)
                                                          (with-message
                                                            (format "include file ~a is newer than ~a"
                                                                    include-req
                                                                    obj-path)
                                                            ($oops/c #f
                                                              ($make-recompile-condition path)
                                                              ...
                                                              obj-path))))))))))
                                       (libdesc-include-req*
                                         (get-library-descriptor found-uid))))))
                           found-uid)]
                        [else
                         ($oops #f
                                "loading ~a did not define library ~s"
               ...
               (let-values ([(src-path obj-path obj-exists?)
                 ...
                 (if src-path
                             ...
                             (if (time>=? (file-modification-time obj-path)
                                 ...
                                 (with-message "object file is older"
                                   (if (and (compile-imported-libraries)
                                            $compiler-is-loaded?)
                                       (with-message
                                         (format "compiling ~s to ~s" src-path obj-path)
                                         (do-compile-library src-path obj-path))
                                       (with-message
                                         (format "loading source file ~s" src-path)
                                         (do-load-library src-path 'load))))))
                         (if (and (compile-imported-libraries)
                                  $compiler-is-loaded?)
                             (with-message
                               ...
                               (let f ([p obj-path])
                                 (let ([p (path-parent p)])
                                   (unless (or (string=? p "")
                                           ...
                                           (f p)
                                           (with-message
                                             (format "creating subdirectory ~s" p)
                                             (mkdir p)))))
                               (do-compile-library src-path obj-path))
                             (with-message
                               (format "loading source file ~s" src-path)
                     ...
                     (if obj-exists?
                         (with-message
                           (format "loading object file ~s" obj-path)
                           (do-load-library obj-path (if ct?
                                                         'load
                                                         'revisit)))
      ...
      (lambda (version-ref version)
        (define sub-version-okay?
          (lambda (x sv)
            (syntax-case x (>= <= and or not)
              ...
              [(<= x) (<= sv #'x)]
              [(and x ...)
               (andmap (lambda (x) (sub-version-okay? x sv)) #'(x ...))]
              [(or x ...)
               (ormap (lambda (x) (sub-version-okay? x sv)) #'(x ...))]
        ...
        (define version-okay?
          (lambda (x v)
            (syntax-case x (and or not)
              [(and x ...)
               (andmap (lambda (x) (version-okay? x v)) #'(x ...))]
              [(or x ...)
               (ormap (lambda (x) (version-okay? x v)) #'(x ...))]
        ...
        (define (do-lookup path tid version-ref)
          (cond
            [($import-library path version-ref #f)
             ...
             (lambda (uid)
               (require-import uid)
               (values (make-resolved-id uid (wrap-marks top-wrap) uid)
                       tid))]
    ...
    (let ()
      (define maybe-get-lib
        (lambda (who libref)
          (syntax-case libref ()
            [(dir-id ... file-id)
             (and (andmap symbol? #'(dir-id ...))
                  (symbol? #'file-id))
             (cond
               [(search-loaded-libraries #'(dir-id ... file-id))
                =>
                (lambda (uid)
                  (and (get-library-descriptor uid) uid))]
               [else #f])]
            [(dir-id ... file-id version-ref)
             (and (andmap symbol? #'(dir-id ...))
                  (symbol? #'file-id)
               ...
               [(search-loaded-libraries #'(dir-id ... file-id))
                ...
                (lambda (uid)
                  (let ([libdesc (get-library-descriptor uid)])
                    (and libdesc
                         (version-okay? #'version-ref
                           (libdesc-version libdesc))
                         uid)))]
               [else #f])]
      ...
      (set-who! library-exports
        (lambda (libref)
          (let* ([binding (lookup-global (get-lib who libref))]
            ...
            (unless (and (eq? (binding-type binding) '$module)
                         (interface? iface))
                ...
                (if (fx= i n)
                    ...
                    (loop (fx+ i 1)
                          (let ([id (vector-ref exports i)])
                            (if (same-marks? (id-marks id) (wrap-marks top-wrap))
      ...
      (set-who! $library-requirements-options
                (make-enumeration
                  '(import visit@visit invoke@visit invoke)))
      (set-who! $make-library-requirements-options
                (enum-set-constructor $library-requirements-options))
                          ...
                          (if (memq uid uid*)
                              ...
                              (gather (cdr req*)
                                ...
                                (let ([path (libreq-path req)]
                                      [version (libreq-version req)])
                                  (cons (if (null? version)
                                            path
                                            `(,@path ,version))
                                        name*))))))))])))
      (set! $system-library?
        (lambda (libref)
          (cond
            [(maybe-get-lib '$system-library? libref)
             =>
             (lambda (uid)
               (libdesc-system? (get-library-descriptor uid)))]
            [else #f]))))
                ...
                (case (libdesc-loaded-import-reqs desc)
                  ...
                  [(#f)
                   ...
                   (on-reset (libdesc-loaded-import-reqs-set! desc #f)
                             (for-each
                               (make-load-req load-import-library path)
                               (libdesc-import-req* desc)))
                                 ...
                                 (let ([n (vector-length x)])
                                   (let vloop ([i 0] [rcinfo* rcinfo*])
                                     (if (fx= i n)
                                         (loop rcinfo*)
                                         (vloop (fx+ i 1)
                                           (scan-outer (vector-ref x i) rcinfo*)))))]
                        ...
                        (let ([rcinfo* (guard (c [else
                          ...
                          (if (and rcinfo*
                              ...
                              (if (compile-imported-libraries)
                                  (guard (c [(and ($recompile-condition? c)
                                                  (eq? ($recompile-importer-path c) #f))
                                             (with-message
                                               (format "recompiling ~s because a dependency has changed"
                                                       ifn)
                                               (handler ifn ofn))])
                                         (for-each
                                           (lambda (rcinfo)
                                             (for-each
                                               (make-load-req load-import-library #f)
                                                               ...
                                                               (let-values
                                                                 ([(src-path obj-path obj-exists?)
                                                                   (library-search
                                                                     ...
                                                                     (library-extensions))])
                                                                 (and obj-exists?
                                                                      (time<=? (file-modification-time obj-path)
                                                                               ofn-mod-time)))])))
                                                        (recompile-info-import-req* rcinfo)))
                                              rcinfo*)
                                      #f
        ...
        (define (s3 src-dir i)
          ...
          (if (fx= i n)
              ...
              (if (char=? (string-ref s i) sep)
                  ...
                  (cons (cons src-dir (make-obj src-dir))
                        (s2 i (fx+ i 1))))))
        (define (s4 src-dir start i)
          ; parsing obj-dir
          (if (fx= i n)
              (list (cons src-dir (substring s start i)))
              (if (char=? (string-ref s i) sep)
                  (cons (cons src-dir (substring s start i))
                        (s5 (fx+ i 1)))
        ...
        (if (null? ls)
            ...
            (let ([x (car ls)])
              (cond
                ...
                [else
                 ($oops who "invalid input-list element ~s" x)])))))
      ...
      (rec library-directories
           ($make-thread-parameter '(("." . "."))
             (lambda (x)
               (cond
                 [(string? x)
                  (parse-string x (library-directories) values)]
      ...
      (rec library-extensions
           ($make-thread-parameter
             ...
             (lambda (x)
               ...
               (cond
                 [(string? x)
                  (parse-string x (library-extensions) default-obj-ext)]
                 [(list? x) (parse-list who x default-obj-ext)]
                 [else
                  ($oops who "invalid extension list ~s" x)]))))))
  ...
  (set-who! $invoke-program
    (lambda (uid th)
      (let ([desc (get-program-descriptor uid)])
        ...
        (for-each
          (lambda (req)
            ($invoke-library (libreq-path req)
              (libreq-version req)
              (libreq-uid req)))
  ...
  (set! $transformer->binding
    (lambda (x) (transformer->binding 'define-syntax x)))
  ...
  (let ()
    (define sc-put-module
      (lambda (exports token new-marks)
        (vector-for-each
          (lambda (id)
            (store-global-subst id token new-marks))
          exports)))
    ...
    (set! $sc-put-cte
      (lambda (id binding top-token)
        (case (binding-type binding)
          [(do-alias)
           (when top-token
                 (store-global-subst id top-token '()))]
          [(do-import)
    ...
    (set! $sc-put-property!
      (lambda (id association propval top-token)
        (when top-token
              (store-global-subst id top-token '()))
        (put-global-definition-hook (cdr association)
          (make-binding 'property propval)))))
        ...
        (let partition ((ls (oblist))
          ...
          (if (null? ls)
              ...
              (let* ((s (car ls)) (m ($sgetprop s '*flags* 0)))
                ...
                (cond
                  [(cond
                     [(and (any-set? (prim-mask
                                       (or system
                                           system-keyword
                                           primitive
                                           keyword))
                                     m)
                           (lookup-global-label s
                             (wrap-marks top-wrap)
                             '*system*))
                      =>
                      (lambda (label)
                        (and (not (eq? label s)) label))]
                     [else #f])
                   =>
                   (lambda (label)
                     (let ([b (get-global-definition-hook label)])
                       (cond
                         ...
                         [else
                          (put-global-definition-hook s b)])))])
                        ...
                        (cond
                          [(any-set? (prim-mask r5rs) m)
                           ...
                           (cond
                             ...
                             [else
                              (repartition id #t #t #f #t)])]
                          [else
                           (repartition id #f #f #f #t)])]
                       [else (repartition id #f #f #f #f)]))]
                  [(any-set? (prim-mask (or primitive system)) m)
                   (put-global-definition-hook s
                     (make-binding 'primitive s))
                   (let ([id (make-resolved-id s (wrap-marks top-wrap) s)])
                     (cond
                        ...
                        (cond
                          [(any-set? (prim-mask r5rs) m)
                           ...
                           (cond
                             ...
                             [else
                              (repartition id #f #t #f #t)])]
                          [else
                           (repartition id #f #f #f #t)])]
                       [else (repartition id #f #f #f #f)]))]
                  [else
                   (partition (cdr ls) r5rs-syntax r5rs ieee scheme system)]))))))
                      ...
                      (lambda (lib)
                        (cond
                          [(assoc lib table)
                           =>
                           (lambda (a)
                             (set-cdr! a (cons (cons unprefixed prim)
                                               (cdr a))))]
                          [else
                           (set! table
                             (cons (cons lib (list (cons unprefixed prim)))
                                   table))]))
                      (priminfo-libraries info))))
                v-prim
                (vector-map get-priminfo v-prim)))
    ...
    (lambda (e r w ae)
      (syntax-case e ()
        [(_ ((var val) ...) e1 e2 ...)
         ...
         (let ([label* (map (lambda (x) (id->label x w))
                            (syntax (var ...)))])
           (for-each
             (lambda (id n)
               (let ([b (lookup n r)])
                   ...
                   (let ([label (car label*)] [b (car b*)])
                     (cond
                       [(if (symbol? label) (lookup-rho r label) label)
                        =>
                        (lambda (label)
                          (let ([old-b (local-label-binding label)]
                                [old-level (local-label-level label)])
                            (local-label-binding-set! label b)
                            (local-label-level-set! label
                              (fxlognot (meta-level)))
              ...
              (syntax-case e ()
                ...
                ((x . y)
                 (let-values ([(xnew maps)
                               (gen-syntax src (syntax x) r maps ellipsis? #f)])
                   (let-values ([(ynew maps)
                                 (gen-syntax src (syntax y) r maps ellipsis? vec?)])
          ...
          (case (car x)
            ...
            ((map)
             (let ((ls (map regen (cdr x))))
               (if (fx= (length ls) 2)
                   (build-call no-source (build-primref 3 'map) ls)
      ...
      (lambda (e r w ae)
        (let ((e (source-wrap e w ae)))
          (syntax-case e ()
            ((_ x)
             (let-values
               ([(e maps) (gen-syntax e (syntax x) r '() ellipsis? #f)])
               (regen e)))
            (_ (syntax-error e)))))))
  ...
  (global-extend 'core '$primitive
    (lambda (e r w ae)
      ...
      (syntax-case e ()
        [(_ x) (id? #'x) (build (fxmax (optimize-level) 2) #'x)]
        [(_ n x)
  ...
  (global-extend 'core 'case-lambda
    (lambda (e r w ae)
      (syntax-case e ()
        ((_ libspec c1 ...)
         (and (eq? (subset-mode) 'system)
              (libspec? (strip (syntax libspec) w)))
         (build-library-case-lambda ae
           (strip (syntax libspec) w)
           (map (lambda (c)
                  (with-values
                    (chi-lambda-clause (source-wrap e w ae) c r w)
                    list))
                (syntax (c1 ...)))))
        ((_ c1 ...)
          ...
          (lambda (p n ids)
            (if (id? p)
                ...
                (syntax-case p ()
                  ...
                  [#&x (let-values ([(p ids) (cvt #'x n ids)])
        ...
        (let ([ids (map car pvars)] [levels (map cdr pvars)])
          (let ([new-vars (map gen-var ids)])
            (let ([labels (map (lambda (var pvar)
                                 (make-local-label
                                   (make-binding 'syntax `(,var . ,(cdr pvar)))
                                   (meta-level)))
                               new-vars
                               pvars)])
          ...
          (cond
            ...
            [(not (andmap (lambda (x) (not (ellipsis? (car x)))) pvars))
             (syntax-error pattern
               (format "misplaced ellipsis in ~s pattern" who))]
            [else
             (let ([y (gen-var 'tmp)])
               (build-let no-source
                 (list y)
    ...
    (global-extend 'core 'syntax-case
      (lambda (e r w ae)
        (define clause
          (lambda (y)
            (syntax-case y ()
              [(pattern template) #'(pattern #t template)]
              [(pattern fender template) #'(pattern fender template)]
        ...
        (lambda (ex*)
          (let f ([ex* ex*] [export* '()] [iexport* '()])
            (if (null? ex*)
                ...
                (let-values ([(export* iexport*)
                              (f (cdr ex*) export* iexport*)])
                  (syntax-case (car ex*) ()
                    [id (identifier? #'id)
                        (values (cons #'id export*) iexport*)]
                    [(id)
                     (identifier? #'id)
                     (values (cons #'id export*) iexport*)]
                    [(id ex ...)
      ...
      (lambda (orig)
        (syntax-case orig ()
          [(_ name exports imports form ...)
           (let-values ([(library-path library-version uid)
                         (create-library-uid #'name)])
             (syntax-case #'exports ()
               [(?export ex ...)
  ...
  (set! sc-expand
    (rec sc-expand
         (case-lambda
           ...
           ((x env records? compiling-a-file)
    ...
    (lambda (x p wr)
      (define get-source
        (lambda (src)
          (call-with-values
            (lambda ()
              ((current-locate-source-object-source) src #t #t))
            (case-lambda
              [()
  ...
  (set! $make-environment
    (lambda (token mutable?)
      (let ([top-ribcage (make-top-ribcage token mutable?)])
        (make-env top-ribcage
          (make-wrap (wrap-marks top-wrap)
            (cons top-ribcage (wrap-subst top-wrap)))))))

  (set! environment? (lambda (x) (env? x)))

  (let ()
    (define tlb?
      (lambda (sym env)
        (cond
          [(top-id-free-label sym
             (wrap-marks top-wrap)
             (env-top-ribcage env))
           =>
           (lambda (label)
             (let ([b (lookup-global label)])
               (case (binding-type b)
                 ...
                 [(global immutable-global)
                  ($top-level-bound? (binding-value b))]
                 [(library-global)
           ...
           (lambda (label)
             (let ([b (lookup-global label)])
               (case (binding-type b)
                 ...
                 [(global immutable-global)
                  (#2%$top-level-value (binding-value b))]
                 [(library-global)
                  (invoke-library (car (binding-value b)))
                  (#2%$top-level-value (cdr (binding-value b)))]
                 [else
                  ($oops 'top-level-value "~s is not a variable" sym)])))]
               ...
               (case (binding-type b)
                 ...
                 [(library-global)
                  ($oops 'set-top-level-value!
                         ...
                         sym)]
                 [else
                  ($oops 'set-top-level-value! "~s is not a variable" sym)])))]
          [else ($oops #f "variable ~s is not bound" sym)])))
           ...
           (lambda (label)
             (let ([b (lookup-global label)])
               (case (binding-type b)
                 ...
                 [else
                  ($oops 'top-level-mutable? "~s is not a variable" sym)])))]
          [else
           ($oops 'top-level-mutable? "variable ~s is not bound" sym)])))
  ...
  (let ()
    ...
    (set! define-top-level-syntax
      (case-lambda
        [(sym val) (dtls sym val (interaction-environment))]
        [(sym val env)
         (unless (environment? env)
                 ($oops who "~s is not an environment" env))
         (dtls sym val env)])))
  ...
  (set! $c-tlv
    (lambda (sym)
      (guard (c [#t ($unbound-object)]) (top-level-value sym))))

  (set! $c-stlv!
    (lambda (sym val)
      (guard (c [#t #f]) (set-top-level-value! sym val) #t)))
            ...
            (lambda (sym)
              (cond
                [(lookup-global-label/pl sym (wrap-marks top-wrap) token)
                 =>
                 (lambda (label/pl)
                   (if (and (symbol? label/pl)
                            (eq? label/pl (make-token:sym token sym)))
                       (let ([new-label (make-token:sym new-token sym)]
                             [b (get-global-definition-hook label/pl)])
                         (let ([id (make-resolved-id sym
                                     (wrap-marks top-wrap)
                                     new-label)])
                           (cond
                             [(or (not b)
                             ...
                             [(and mutable?
                              ...
                              ($sc-put-cte id
                                ...
                                new-token)]
                             [else
                              ($sc-put-cte id b new-token)])))
                       (store-global-subst
                         (make-resolved-id sym
                           (wrap-marks top-wrap)
                           label/pl)
                         new-token
                         '())))]))
    ...
    (lambda import-spec*
      (define eval-import
        (lambda (orig env)
          (lambda (import-spec)
            (top-level-eval-hook
              (at-top (parameterize ([meta-level 0])
                        (chi-top* `(,(wrap '$import (env-wrap ($system-environment)))
                                    ,orig
                                    ,(wrap (list import-spec) (env-wrap env))
                                    #f
                                    #f)
                                  empty-wrap
    ...
    (lambda (name env)
      (if (gensym? name)
          (let ([pretty-name (string->symbol (symbol->string name))])
            (if (eq? (lookup-global-label pretty-name
                       (wrap-marks top-wrap)
  ...
  (set! ieee-environment
    (let ([r ($make-environment '*ieee* #f)]) (lambda () r)))
  ...
  (let ()
    (define d->s
      (lambda (id datum who)
        (unless (nonsymbol-id? id)
                ($oops who "~s is not an identifier" id))
        ; no longer transfering annotation, since this can produce
        ; misleading profile output
        (make-syntax-object datum (syntax-object-wrap id))))
    (set-who! datum->syntax (lambda (id datum) (d->s id datum who)))
    (set-who! datum->syntax-object (lambda (id datum) (d->s id datum who))))
  ...
  (set! $distinct-bound-ids?
    (lambda (ids) (distinct-bound-ids? ids)))
  ...
  (set-who! make-compile-time-value
            (lambda (x) ($make-compile-time-value x)))

  (set-who! compile-time-value? (lambda (x) ($compile-time-value? x)))

  (set-who! compile-time-value-value
    (lambda (x)
      (unless ($compile-time-value? x)
              ($oops who "~s is not a compile-time value" x))
    ...
    (lambda (x)
      (let f ([x x] [n 0] [k (lambda () (values #f #t))])
        (cond
          ...
          [(fx= n 3) (k)]
          [(pair? x)
           (f (car x) (fx+ n 1) (lambda () (f (cdr x) (fx+ n 1) k)))]
          [(vector? x)
           (if (fx= (vector-length x) 0)
               (k)
               (f (vector-ref x 0) (fx+ n 1) k))]
          [else (k)]))))
  ...
  (let ()
    ...
    (define match-each
      (lambda (e p w)
        (cond
          ((annotation? e) (match-each (annotation-expression e) p w))
          ((pair? e)
    ...
    (define match-each-any
      (lambda (e w)
        (cond
          ((annotation? e) (match-each-any (annotation-expression e) w))
          ((pair? e)
    ...
    (set! $noexpand?
      (lambda (x) (and (pair? x) (equal? (car x) noexpand))))))
...
(define-syntax with-syntax
  (lambda (x)
    (syntax-case x ()
      ((_ () e1 e2 ...) (syntax (let () e1 e2 ...)))
      ((_ ((out in)) e1 e2 ...)
       (syntax (syntax-case in () (out (let () e1 e2 ...)))))
      ((_ ((out in) ...) e1 e2 ...)
       (syntax (syntax-case (list in ...) () ((out ...) (let () e1 e2 ...))))))))
...
(define-syntax datum
  (syntax-rules () [(_ x) (syntax->datum (syntax x))]))
  ...
  (lambda (x)
    (define (helper clause* els?)
      (let ([sort? ($profile-source-data?)])
        ...
        #`(cond
            #,@(map clause-clause
                    (sort-em (map parse-clause clause*)))
            .
            #,els?)))
    (syntax-case x (else)
      [(_ m1 ... [else e1 e2 ...])
       (helper #'(m1 ...) #'([else e1 e2 ...]))]
  ...
  (let ()
    (define (quasi p lev)
      (syntax-case p (unquote quasiquote)
        ...
        [(quasiquote p)
         (quasicons #'("quote" quasiquote) (quasi #'(p) (+ lev 1)))]
        [(p . q)
         (syntax-case #'p (unquote unquote-splicing)
           ...
           [(unquote-splicing p ...)
            (if (= lev 0)
                ...
                (quasicons
                  (quasicons #'("quote" unquote-splicing)
                    (quasi #'(p ...) (- lev 1)))
                  (quasi #'q lev)))]
    ...
    (define (quasivector x)
      (syntax-case x ()
        [("quote" (x ...)) #'("quote" #(x ...))]
        [_ (let f ([y x] [k (lambda (ls) #`("vector" #,@ls))])
             (syntax-case y ()
               [("quote" (y ...)) (k #'(("quote" y) ...))]
               [("list" y ...) (k #'(y ...))]
               [("list*" y ... z)
                (f #'z (lambda (ls) (k (append #'(y ...) ls))))]
               [else #`("list->vector" #,x)]))]))
    (define (quasibox x)
      (syntax-case #'x ()
        [("quote" x) #'("quote" #&x)]
...
(define-syntax delay
  (lambda (x)
    (syntax-case x ()
      ((delay exp) (syntax ($make-promise (lambda () exp)))))))
         ...
         ((constructor
          ...
          ((assign ...)
           (map (lambda (x)
                ...
                (syntax (id1 ... id2 ...))))
          (structure-length (fx+ (length (syntax (id1 ... id2 ...))) 1))
          ((index ...)
...
(when-feature pthreads
  (define-syntax with-mutex
    (syntax-rules ()
      ((_ m-expr e1 e2 ...)
       (let ([m m-expr])
         (dynamic-wind (lambda () (mutex-acquire m))
           (lambda () e1 e2 ...)
    ...
    (define reconstitute-formals
      (lambda (fmls tmps)
        (syntax-case fmls ()
          ...
          [(id . fmls)
           (cons (car tmps)
                 (reconstitute-formals #'fmls (cdr tmps)))])))
    (define make-temp
      (lambda (id)
        ; return like-named gensym to make debugging easier
        (datum->syntax #'* (gensym (symbol->string (syntax->datum id))))))
    (define domvlet
...
(define-syntax rec
  (lambda (x)
    (syntax-case x ()
      ((_ x v) (identifier? (syntax x)) (syntax (letrec ((x v)) x))))))
        ...
        (syntax-case p ()
          ((id . p)
           (with-syntax ((body (build-rc-body (syntax p) body)))
             (syntax (let ((rec (cdr rec))) (let ((id (car rec))) body)))))
          (() (with-syntax ((body body)) (syntax (begin . body))))
          (id (with-syntax ((body body))
      ...
      (lambda (x)
        (syntax-case x ()
          ((_ tag keys idspec body rest)
           (syntax (with-syntax ((body (build-rc-body (syntax idspec) (syntax body))))
                     (syntax (if (memv tag 'keys) body . rest))))))))
    (lambda (x)
      (syntax-case x ()
        ((_ e m1 m2 ...)
         (with-syntax
                    ...
                    (if (null? clauses)
                        ...
                        (with-syntax ((rest (f (car clauses) (cdr clauses))))
                          (syntax-case clause (else)
                            ...
                            (_ (syntax-error x))))))))
           (syntax (let ((rec e)) (let ((tag (car rec))) body)))))))))
...
(define-syntax unless
  (syntax-rules ()
    ((_ e0 e1 e2 ...) (if (not e0) (begin e1 e2 ...)))))
...
(define-syntax extend-syntax
  (lambda (x)
    ...
    (define pattern-variable?
      (lambda (sym ids) (memq sym (map id-name ids))))
             ...
             ((and (pair? (cdr exp)) (eq? (cadr exp) '...))
              (let ((x (loop)))
                (gen-cons (list 'unquote-splicing
                                (make-loop x
                                  (gen keys (car exp) ids (cons x loops) qqlev)))
          ...
          `(lambda (,x)
             (cond
               ,@(map (lambda (cl) (make-clause keys cl x))
                      clauses)
               (else
                (assertion-violationf ',(car keys) "invalid syntax ~s" ,x)))))))
...
(define $fp-filter-type
  (lambda (type void-okay?)
    ...
    (case type
      ...
      [(ptr) 'scheme-object]
      [(iptr)
       (constant-case ptr-bits [(32) 'integer-32] [(64) 'integer-64])]
      [(uptr)
       (constant-case ptr-bits [(32) 'unsigned-32] [(64) 'unsigned-64])]
      [(void*)
       (constant-case ptr-bits [(32) 'unsigned-32] [(64) 'unsigned-64])]
      [(int)
       (constant-case int-bits [(32) 'integer-32] [(64) 'integer-64])]
      [(unsigned unsigned-int)
       (constant-case int-bits [(32) 'unsigned-32] [(64) 'unsigned-64])]
      [(short)
       (constant-case short-bits [(16) 'integer-16] [(32) 'integer-32])]
      [(unsigned-short)
       (constant-case short-bits [(16) 'unsigned-16] [(32) 'unsigned-32])]
      [(long)
       (constant-case long-bits [(32) 'integer-32] [(64) 'integer-64])]
      [(unsigned-long)
       (constant-case long-bits [(32) 'unsigned-32] [(64) 'unsigned-64])]
      [(long-long) (constant-case long-long-bits [(64) 'integer-64])]
      [(unsigned-long-long)
       (constant-case long-long-bits [(64) 'unsigned-64])]
      [(size_t)
       (constant-case size_t-bits [(32) 'unsigned-32] [(64) 'unsigned-64])]
      [(ssize_t)
       (constant-case size_t-bits [(32) 'integer-32] [(64) 'integer-64])]
      [(ptrdiff_t)
       (constant-case ptrdiff_t-bits [(32) 'integer-32] [(64) 'integer-64])]
      [(wchar_t) 'wchar]
      [(float) 'single-float]
      [(double) 'double-float]
      [(string) 'utf-8]
       ...
       (constant-case wchar-bits
         ...
         [(32)
          (constant-case native-endianness
            ...
            [(big) 'utf-32be])])]
      [else
       (and (or ($ftd? type) ($ftd-as-box? type)) type)])))
  ...
  (lambda (type)
    (cond
      [(assq type foreign-datatypes)
       =>
       (lambda (a)
         (apply (lambda (spec bytes pred) pred) a))]
      [else
       (case type
         [(boolean void) '(lambda (id) #t)]
         [(char)
          '(lambda (id)
             (and (char? id) (fx<= (char->integer id) #xff)))]
         [(wchar)
          (constant-case wchar-bits
            [(16)
             '(lambda (id)
                (and (char? id)
                     (fx<= (char->integer id) #xffff)))]
            [(32) '(lambda (id) (char? id))])]
         [(utf-8 utf-16le utf-16be utf32-le utf32-be)
          '(lambda (id) (or (not id) (string? id)))]
         [(u8* u16* u32*) '(lambda (id) (or (not id) (bytevector? id)))]
         [(fixnum) '(lambda (id) (fixnum? id))]
         [else
          ($oops '$fp-type->pred "unrecognized type ~s" type)])])))
      ...
      (cond
        ...
        [else
         (let* ([orig-c (car conv*)]
           ...
           (when (member c accum)
                 (syntax-error orig-c
                   (format "redundant ~s convention" who)))
           (unless (or (null? accum)
                       (eq? c 'adjust-active)
                       (and (eq? 'adjust-active (car accum))
                            (null? (cdr accum))))
                   (syntax-error orig-c
                     (format "conflicting ~s convention" who)))
           (loop (cdr conv*)
                 (cons c accum)
                 (if c (cons c keep-accum) keep-accum)))]))))
          ...
          ([(((check ...) (actual ...) (arg ...)) ...)
                     ...
                     (or (case type
                         ...
                         (if (or ($ftd? type) ($ftd-as-box? type))
                             ...
                             (with-syntax
                               ...
                               #`(#,(if unsafe?
                                        #'()
                                        #'((unless (pred x) (err ($moi) x)))) (x) (type)))))))
                 type*
                 #'(t ...))]
           [(result-filter result)
            (case result-type
              [(boolean)
               #`((lambda (x) (not (eq? x 0)))
                  #,(constant-case int-bits
                      ...
                      [(64) #'integer-64]))]
              [(char)
               #'((lambda (x) (#3%integer->char (#3%fxlogand x #xff))) unsigned-8)]
              [(wchar)
               #`(integer->char
                   #,(constant-case wchar-bits
                       [(16) #'unsigned-16]
              ...
              [(utf-16le)
               #'((lambda (x) (and x (utf16->string x 'little #t))) u16*)]
              [(utf-16be)
               #'((lambda (x) (and x (utf16->string x 'big #t))) u16*)]
              [(utf-32le)
               #'((lambda (x) (and x (utf32->string x 'little #t))) u32*)]
              [(utf-32be)
               #'((lambda (x) (and x (utf32->string x 'big #t))) u32*)]
              [(integer-24)
               #`((lambda (x)
                    (#,(constant-case ptr-bits
                         [(32) #'mod0]
                         [(64) #'fxmod0]) x #x1000000)) integer-32)]
              [(unsigned-24)
               #`((lambda (x)
                    (#,(constant-case ptr-bits [(32) #'mod] [(64) #'fxmod]) x #x1000000)) unsigned-32)]
              [(integer-40)
               #`((lambda (x) (mod0 x #x10000000000)) integer-64)]
              [(unsigned-40)
               #`((lambda (x) (mod x #x10000000000)) unsigned-64)]
              [(integer-48)
               #`((lambda (x) (mod0 x #x1000000000000)) integer-64)]
              [(unsigned-48)
               #`((lambda (x) (mod x #x1000000000000)) unsigned-64)]
              [(integer-56)
               #`((lambda (x) (mod0 x #x100000000000000)) integer-64)]
              [(unsigned-56)
               #`((lambda (x) (mod x #x100000000000000)) unsigned-64)]
              [else
               (cond
                 [($ftd-as-box? result-type)
                  ...
                  #`((lambda (r) (void))
                     #,(datum->syntax #'foreign-procedure result-type))]
                 [else
    ...
    (define filter-type
      (lambda (r x result?)
        (let ([what (if result? 'result 'argument)])
          (or ($fp-filter-type
                ($expand-fp-ftype 'foreign-procedure what r x)
                result?)
              (syntax-error x
                (format "invalid foreign-procedure ~s type specifier" what))))))
    (syntax-case x ()
      [(_ c ... ?name (arg ...) result)
       (lambda (r)
         ($make-foreign-procedure 'foreign-procedure
           ...
           #'($foreign-entry ?name)
           (map (lambda (x) (filter-type r x #f))
                #'(arg ...))
          ...
          ([((actual (t ...) (arg ...)) ...)
            (map (lambda (type)
                   (or (case type
                         ...
                         [(utf-8)
                          (with-syntax ([(x) (generate-temporaries #'(*))])
                            #`((and x (utf8->string x)) (x) (u8*)))]
                         [(utf-16le)
                          (with-syntax ([(x) (generate-temporaries #'(*))])
                            #`((and x (utf16->string x 'little #t)) (x) (u16*)))]
                         [(utf-16be)
                          (with-syntax ([(x) (generate-temporaries #'(*))])
                            #`((and x (utf16->string x 'big #t)) (x) (u16*)))]
                         [(utf-32le)
                          (with-syntax ([(x) (generate-temporaries #'(*))])
                            #`((and x (utf32->string x 'little #t)) (x) (u32*)))]
                         [(utf-32be)
                          (with-syntax ([(x) (generate-temporaries #'(*))])
                            #`((and x (utf32->string x 'big #t)) (x) (u32*)))]
                         [(integer-24)
                          (with-syntax ([(x) (generate-temporaries #'(*))])
                            #`((mod0 x #x1000000) (x) (integer-32)))]
                         [(unsigned-24)
                          (with-syntax ([(x) (generate-temporaries #'(*))])
                            #`((mod x #x1000000) (x) (unsigned-32)))]
                         [(integer-40)
                          (with-syntax ([(x) (generate-temporaries #'(*))])
                            #`((mod0 x #x10000000000) (x) (integer-64)))]
                         [(unsigned-40)
                          (with-syntax ([(x) (generate-temporaries #'(*))])
                            #`((mod x #x10000000000) (x) (unsigned-64)))]
                         [(integer-48)
                          (with-syntax ([(x) (generate-temporaries #'(*))])
                            #`((mod0 x #x1000000000000) (x) (integer-64)))]
                         [(unsigned-48)
                          (with-syntax ([(x) (generate-temporaries #'(*))])
                            #`((mod x #x1000000000000) (x) (unsigned-64)))]
                         [(integer-56)
                          (with-syntax ([(x) (generate-temporaries #'(*))])
                            #`((mod0 x #x100000000000000) (x) (integer-64)))]
                         [(unsigned-56)
                          (with-syntax ([(x) (generate-temporaries #'(*))])
                            #`((mod x #x100000000000000) (x) (unsigned-64)))]
                         [else #f])
                       (with-syntax ([(x) (generate-temporaries #'(*))])
                         #`(x (x)
                              (#,(datum->syntax #'foreign-callable type))))))
                 type*)]
                 ...
                 [($ftd-as-box? result-type)
                  ...
                  (with-syntax
                    ([type (datum->syntax #'foreign-callable result-type)]
                     [ftd (datum->syntax #'foreign-callable
                            (unbox result-type))])
                    #`((lambda (x) (void))
                       ; callable result is ignored
                       type
                       [ftd]
                       [&-result]))]
                 [else
                  (with-syntax
                    ([pred (datum->syntax #'foreign-callable
                             ($fp-type->pred result-type))]
                     [type (datum->syntax #'foreign-callable result-type)])
                    #`((lambda (x)
                         #,@(if unsafe?
                                #'()
  ...
  (lambda (x)
    (define filter-type
      (lambda (r x result?)
        (let ([what (if result? 'result 'argument)])
          (or ($fp-filter-type
                ($expand-fp-ftype 'foreign-callable what r x)
                result?)
              (syntax-error x
                (format "invalid foreign-callable ~s type specifier" what))))))
    (syntax-case x ()
      [(_ c ... ?proc (arg ...) result)
       (lambda (r)
    ...
    (define (help clause*)
      (with-syntax ([([e0 e1 e2 ...] ...) clause*])
        (with-syntax ([(t ...) (generate-temporaries #'(e0 ...))])
          #'(let-syntax
              ([a (cond [e0 (syntax-rules () [(__ t ...) t])] ...)])
              (a (begin e1 e2 ...) ...)))))
    (syntax-case x (else)
      [(_ [e0 e1 e2 ...] ... [else ee1 ee2 ...])
       (help #'([e0 e1 e2 ...] ... [else ee1 ee2 ...]))]
      [(k [e0 e1 e2 ...] ...) (help #'([e0 e1 e2 ...] ... [else (void)]))])))
           ...
           [(and (pair? info)
            ...
            (with-syntax ([(rtd rcd sealed? protocol?) (cdr info)])
              (if (record-type-descriptor? #'rtd)
                  ...
                  #'rtd))]
           [else
            (syntax-error #'name "type-descriptor: unrecognized record")]))])))
       ...
       (lambda (r)
         ...
         (cond
           [(and (pair? info)
                 (eq? (car info) '#{r6rs-record vc7pishgmrh09qm-a}))
            (with-syntax ([(rtd rcd sealed? protocol?) (cdr info)])
              (if (record-constructor-descriptor? #'rcd)
                  #''rcd
                  #'rcd))]
           [(and (pair? info)
                 (eq? (car info) '#{record val9xfsq6oa12q4-a}))
        ...
        (option lambda
          ...
          (if (null? ols)
              ...
              (syntax-case (car ols) (constructor predicate prefix)
                [(prefix s)
                 (string? (datum s))
                 (option parse-options ((pref-id (datum s))) (cdr ols))]
                [(predicate id)
                 (identifier? #'id)
      ...
      (define field->id
        ...
        (lambda (f)
          ...
          (syntax-case f ()
            ...
            [(id) (okay? #f #f #'id) #'id]
            [(class/type id)
             (or (okay? #'class/type #f #'id)
                 (okay? #f #'class/type #'id))
             #'id]
                          ...
                          (let* ([pids (with ([((pid ...) ...) #'((pid1 ... pid2 ...) ...)])
                            ...
                            (with ([((access ordinal) ...)
                                    (map (lambda (id ordinal)
                                           (list (construct-name #'name #'prefix id) ordinal))
                                         (list-tail allids npids)
                                         (list-tail (enumerate allids) npids))]
                                   [((assign !ordinal) ...)
                                    (let f ([ids (list-tail allids npids)] [ordinal npids])
                                      (if (null? ids)
                                          '()
                                          (if (csv7:record-field-mutable? #'rtd ordinal)
                                              (cons (list (construct-name
                                                            #'name
                                                            "set-"
                                                            #'prefix
                                                            (car ids)
          ...
          (Mclause lambda
            ...
            (let ([%mutable-fields (filter field-desc-mutator %fields)])
              (with-syntax ([primlev (if (= (optimize-level) 3) 3 2)]
                            [(accessor-name ...)
                             (map field-desc-accessor
                                  %fields)]
                            [(accessor-index ...)
                             (map field-desc-index %fields)]
                            [(mutator-name ...)
                             (map field-desc-mutator
                                  %mutable-fields)]
                            [(mutator-index ...)
                             (map field-desc-index
                                  %mutable-fields)])
                (unless ($distinct-bound-ids?
                          `(,make-name ,pred-name
                             ,@#'(accessor-name ...)
                             ,@#'(mutator-name ...)))
                        (syntax-error
                ...
                (if (and (or %uid ($syntax-top-level?))
                    ...
                    (let ([rtd ($make-record-type-descriptor #!base-rtd
                                 ...
                                 (list->vector
                                   (map (lambda (%field)
                                          (syntax->datum (field-desc-spec %field)))
                                        %fields))
                                 'define-record-type)])
                      (if %protocol
                          #`(begin
                              ...
                              (define #,make-name
                                (($primitive primlev r6rs:record-constructor) rcd))
                              (define #,pred-name
                                (($primitive primlev record-predicate) '#,rtd))
                              (define accessor-name
                                (($primitive primlev record-accessor)
                                 '#,rtd
                                 accessor-index))
                              ...
                              (define mutator-name
                                (($primitive primlev record-mutator)
                                 '#,rtd
                                 mutator-index))
                              ...)
...
#;
(define-syntax immutable
  ; defined above for define-record
  (lambda (x) (syntax-error x "misplaced aux keyword")))
#;
(define-syntax mutable
  ; defined above for define-record
    ...
    (syntax-case x ()
      [(_ type-name (symbol ...) constructor-syntax)
       ...
       #'(module ((type-name expand-time-this-enum)
           ...
           (define this-enum
             (make-enumeration '(symbol ...)))
           (define-syntax expand-time-this-enum
             (make-compile-time-value
               `(#{enum-set yb11fsqj62y93q3-a} .
                  ,(make-enumeration '(symbol ...)))))
           (define-syntax type-name
             (lambda (x)
               (lambda (r)
                 (let ([expand-time-this-enum
                         (cdr (r #'expand-time-this-enum))])
                   (syntax-case x ()
                     [(_ x)
                      (identifier? #'x)
                      (begin
    ...
    (syntax-case x ()
      [(_ id ...)
       ...
       (begin
         ...
         (with-syntax
           ([members ($enum-set-members
                       ($make-fasl-strip-options (datum (id ...))))])
    ...
    (syntax-case x ()
      [(_ id ...)
       ...
       (begin
         ...
         (with-syntax
           ([members ($enum-set-members
                       ($make-annotation-options (datum (id ...))))])
    ...
    (syntax-case x ()
      [(_ id ...)
       ...
       (begin
         ...
         (with-syntax
           ([members ($enum-set-members
                       ($make-library-requirements-options (datum (id ...))))])
...
(define-syntax assert
  (lambda (x)
    (syntax-case x ()
      [(_ expr)
       #`(or expr
             #,($make-source-oops #f "failed assertion" #'expr))])))
  ...
  (set-who! make-source-object
    (case-lambda
      [(sfd bfp efp)
       ...
       (unless (if (fixnum? bfp)
                   (fx>= bfp 0)
                   (and (bignum? bfp) ($bigpositive? bfp)))
               ($oops who "~s is not an exact nonnegative integer" bfp))
       (unless (if (fixnum? efp)
                   (fx>= efp 0)
      ...
      [(sfd bfp efp line column)
       ...
       (unless (if (fixnum? efp)
                   ...
                   (and (bignum? efp) ($bigpositive? efp)))
               ($oops who "~s is not an exact nonnegative integer" efp))
       (unless (if (fixnum? line)
                   (fx>= line 1)
                   (and (bignum? line) ($bigpositive? line)))
               ($oops who "~s is not an exact positive integer" line))
       (unless (if (fixnum? column)
                   (fx>= column 1)
                   (and (bignum? column)
                        ($bigpositive? column)))
  ...
  (set-who! source-file-descriptor
    (lambda (path checksum)
      ...
      (unless (if (fixnum? checksum)
                  (fx>= checksum 0)
                  (and (bignum? checksum)
                       ($bigpositive? checksum)))
              ($oops who "~s is not an exact nonnegative integer" checksum))
      (%make-source-file-descriptor path
        (ash checksum -16)
        (logand checksum #xffff))))
  (set-who! source-file-descriptor?
            (lambda (x) (%source-file-descriptor? x)))
  (set-who! source-file-descriptor-path
    (lambda (x)
      (unless (%source-file-descriptor? x)
              ($oops who "~s is not a source-file descriptor" x))
      (%source-file-descriptor-name x)))
  (set-who! source-file-descriptor-checksum
    (lambda (x)
      (unless (%source-file-descriptor? x)
              ($oops who "~s is not a source-file descriptor" x))
      (logor (ash (%source-file-descriptor-length x) 16)
      ...
      (cond
        ...
        [(%source? src)
         ($locate-source (%source-sfd src)
           ...
           cache?)]
        [else ($oops who "~s is not a source object" src)])))
...
(set-who! $make-annotation-options
          (enum-set-constructor $annotation-options))