...
(define-syntax constant
  (lambda (x)
    (syntax-case x ()
      ((_ x)
       (identifier? #'x)
       #`'#,(datum->syntax #'x (lookup-constant (datum x)))))))
...
(define-syntax type-case
  (syntax-rules (else)
    [(_ expr [(pred1 pred2 ...) e1 e2 ...] ... [else ee1 ee2 ...])
     (let ([t expr])
       (cond
...
(define-syntax float-type-case
  (lambda (x)
    (syntax-case x (ieee else)
      ((_ ((ieee tag ...) e1 e2 ...) m ...)
       #t
       ; all currently supported machines are ieee
       #'(begin e1 e2 ...))
      ((_ ((tag1 tag2 ...) e1 e2 ...) m ...)
       #'(float-type-case ((tag2 ...) e1 e2 ...) m ...))
      ((_ (() e1 e2 ...) m ...) #'(float-type-case m ...))
...
(define-syntax make-libspec-flags
  (lambda (x)
    (syntax-case x ()
      [(_ index-base
          does-not-expect-headroom?
          closure?
          interface
          error?
          has-does-not-expect-headroom-version?)
       #'(begin
           (unless (fx>= (- (expt 2 (constant libspec-index-base-size)) 1)
                         index-base
                         0)
           ...
           (fxlogor (if does-not-expect-headroom?
             ...
             (if closure?
                 (fxsll 1 (constant libspec-closure-index))
                 0)
             (if error?
                 (fxsll 1 (constant libspec-error-index))
                 0)
             (if has-does-not-expect-headroom-version?
                 (fxsll 1
                   (constant libspec-has-does-not-expect-headroom-version-index))
                 0)))])))

(define-syntax libspec-does-not-expect-headroom?
  (syntax-rules ()
    [(_ ?libspec)
     (fxbit-set? (libspec-flags ?libspec)
       (constant libspec-does-not-expect-headroom-index))]))
  ...
  (syntax-rules ()
    [(_ ?libspec)
     (fxbit-field (libspec-flags ?libspec)
       ...
       (fx+ (constant libspec-interface-size)
            (constant libspec-interface-offset)))]))
...
(define-syntax return-values
  (syntax-rules () ((_ args ...) (values args ...))))

(define-syntax with-values
  (syntax-rules ()
    ((_ producer proc) (call-with-values (lambda () producer) proc))))
    ...
    (syntax-case x ()
      [(_ e)
       #`(let-syntax
           ([t (if e
                   (lambda () #'(void))
                   #,(#%$make-source-oops #f "failed meta-assertion" #'e))])
           (void))])))
...
(define-syntax features
  (lambda (x)
    (syntax-case x ()
      [(k foo ...)
       (with-implicit
         (k feature-list when-feature unless-feature if-feature)
         #'(begin
...
(define-constant machine-type-name
  (cdr (assv (constant machine-type) (constant machine-type-alist))))

(define-syntax log2
  (syntax-rules () [(_ n) (integer-length (- n 1))]))
    ...
    (syntax-case x ()
      [(_ reloc-name ...)
       (with-syntax ([(i ...) (enumerate #'(reloc-name ...))])
         #'(begin (define-constant reloc-name i) ...))])))
...
(define-reloc-constants (all reloc-abs)
  (x86 reloc-rel)
...
(define-constant STRVNCATE #x0001)
; V for U to avoid msvc errno.h conflict
...
(define-constant PORT-FLAG-INPUT
  (ash (constant port-flag-input) (constant port-flags-offset)))
(define-constant PORT-FLAG-OUTPUT
  (ash (constant port-flag-output) (constant port-flags-offset)))
(define-constant PORT-FLAG-BINARY
  (ash (constant port-flag-binary) (constant port-flags-offset)))
(define-constant PORT-FLAG-CLOSED
  (ash (constant port-flag-closed) (constant port-flags-offset)))
(define-constant PORT-FLAG-FILE
  (ash (constant port-flag-file) (constant port-flags-offset)))
(define-constant PORT-FLAG-COMPRESSED
  (ash (constant port-flag-compressed) (constant port-flags-offset)))
(define-constant PORT-FLAG-EXCLUSIVE
  (ash (constant port-flag-exclusive) (constant port-flags-offset)))
(define-constant PORT-FLAG-BOL
  (ash (constant port-flag-bol) (constant port-flags-offset)))
(define-constant PORT-FLAG-EOF
  (ash (constant port-flag-eof) (constant port-flags-offset)))
(define-constant PORT-FLAG-BLOCK-BUFFERED
  (ash (constant port-flag-block-buffered) (constant port-flags-offset)))
(define-constant PORT-FLAG-LINE-BUFFERED
  (ash (constant port-flag-line-buffered) (constant port-flags-offset)))
(define-constant PORT-FLAG-INPUT-MODE
  (ash (constant port-flag-input-mode) (constant port-flags-offset)))
(define-constant PORT-FLAG-CHAR-POSITIONS
  (ash (constant port-flag-char-positions) (constant port-flags-offset)))
(define-constant PORT-FLAG-R6RS
  (ash (constant port-flag-r6rs) (constant port-flags-offset)))
(define-constant PORT-FLAG-FOLD-CASE
  (ash (constant port-flag-fold-case) (constant port-flags-offset)))
(define-constant PORT-FLAG-NO-FOLD-CASE
  (ash (constant port-flag-no-fold-case) (constant port-flags-offset)))
...
;;; allocation spaces
(define-constant space-locked #x20)
; lock flag
(define-constant space-old #x40)
; oldspace flag
         ...
         (with-syntax ([(name ...)
                       ...
                       [(value ...)
                        #'(real-value
                            ...
                            ...
                            last-unreal-value)])
           (with-syntax ([(space-name ...)
                          (map (lambda (n) (construct-name n "space-" n))
                               #'(name ...))])
             (unless (< (syntax->datum #'last-unreal-value)
                        (constant space-locked))
                     ($oops 'define-alloc-spaces
                            "conflict with space-locked"))
             (unless (< (syntax->datum #'last-unreal-value)
                        (constant space-old))
             ...
             #'(begin
                 ...
                 ...
                 (define-constant real-space-alist
...
(define-alloc-spaces
  (real (swept (new "new" #\n 0)
          ; all generation 0 objects allocated here
          (impure "impure" #\i 1)
          ; most mutable objects allocated here (all ptrs)
          (symbol "symbol" #\x 2)
          ;
          (port "port" #\q 3)
          ;
          ...
          ;
          (pure "pure" #\p 6)
          ; swept immutable objects allocated here (all ptrs)
        ...
        ;
        (unswept (data "data" #\d 11)))
  ; unswept objects allocated here
  (unreal (empty "empty" #\e 12)))
; available segments
...
(define-constant type-fixnum 0)
; #b100/#b000 32-bit, #b000 64-bit
...
(define-constant type-other-number #b0110)
; bit 3 reset for numbers
(define-constant type-bignum #b00110)
; bit 4 reset for bignums
(define-constant type-positive-bignum #b000110)
(define-constant type-negative-bignum #b100110)
(define-constant type-ratnum #b00010110)
; bit 4 set for non-bignum numbers
(define-constant type-inexactnum #b00110110)
(define-constant type-exactnum #b01010110)
(define-constant type-box #b0001110)
; bit 3 set for non-numbers
(define-constant type-immutable-box #b10001110)
; low 7 bits match `type-box`
...
(define-constant fixnum-offset
  (- (constant ptr-bits) (constant fixnum-bits)))
...
(define-constant type-io-port
  (fxlogor (constant type-input-port) (constant type-output-port)))
(define-constant type-system-code
  (fxlogor (constant type-code)
    (fxsll (constant code-flag-system) (constant code-flags-offset))))
(define-constant type-continuation-code
  (fxlogor (constant type-code)
    (fxsll (constant code-flag-continuation) (constant code-flags-offset))))
...
(define-constant mask-signed-bignum
  (fxlogor (constant mask-bignum) (constant mask-bignum-sign)))
...
(define-constant mask-textual-output-port
  (constant mask-binary-output-port))
...
(define-constant mask-continuation-code
  (fxlogor (fxsll (constant code-flag-continuation)
...
(define-constant vector-length-factor
  (expt 2 (constant vector-length-offset)))
(define-constant string-length-factor
  (expt 2 (constant string-length-offset)))
(define-constant bignum-length-factor
  (expt 2 (constant bignum-length-offset)))
(define-constant fxvector-length-factor
  (expt 2 (constant fxvector-length-offset)))
(define-constant bytevector-length-factor
  (expt 2 (constant bytevector-length-offset)))
                ...
                `((ptr . scheme-object)
                  ...
                  (unsigned-long .
                    ,(constant-case long-bits
                       ...
                       [(64) 'unsigned-64]))
                  (long-long .
                    ,(constant-case long-long-bits [(64) 'integer-64]))
                  (unsigned-long-long .
                  ...
                  (double . double-float)))])
      (syntax-rules ()
        [(_ ?x) (let ([x ?x]) (cond [(assq x 'alist) => cdr] [else x]))])))
                ...
                `((byte . signed-8)
                  ...
                  (bigit .
                    ,(constant-case bigit-bits
                       ...
                       [(32) 'unsigned-32]))
                  (string-char .
                    ,(constant-case string-char-bits [(32) 'unsigned-32]))))])
      (syntax-rules ()
        [(_ ?x) (let ([x ?x]) (cond [(assq x 'alist) => cdr] [else x]))]))))
    ...
    (define parse-field
      (lambda (field)
        (syntax-case field (constant)
          [(field-type field-name) (list #'field-type #'field-name #f)]
          [(field-type field-name n)
           (integer? (datum n))
           (list #'field-type #'field-name (datum n))]
          [(field-type field-name (constant sym))
           (list #'field-type #'field-name (lookup-constant (datum sym)))])))
    (syntax-case x ()
         ...
         (with-values
           ...
           (lambda (pm mpm flds size)
             (let ([var? (eq? (car (last-pair #'(field-length ...))) 0)])
               (with-syntax
                 ([(name-field-disp ...)
                   (map (lambda (fn) (make-name-field-disp #'name fn))
                        (datum (field-name ...)))]
                  [(field-disp ...)
...
(define-primitive-structure-disps ephemeron type-pair
  ([ptr car]
   [ptr cdr]
   [ptr next]
   ; `next` is needed by the GC to keep track of pending ephemerons
   [ptr trigger-next]))
...
(define-primitive-structure-disps flonum type-flonum ([double data]))
        ...
        (f (cdr ls)
           (let* ([sym (car ls)]
             ...
             (if (and (> n 8)
                 ...
                 (cons (string->symbol (substring str 3 (- n 5)))
                       params)
                 params))))))
            ...
            (let ((fld (car flds)))
              (cond
                [(< b (fld-byte fld))
                 (cons 0
                       (f flds (+ b (constant ptr-bytes)) e*))]
                [(> b (fld-byte fld)) (f (cdr flds) b (cdr e*))]
                [else
                 ; (= b (fld-byte fld))
                 (cons (if (eq? (filter-foreign-type (fld-type fld))
                                'scheme-object)
                           (car e*)
                           0)
...
(define-syntax string-char-type
  (lambda (x)
    (with-syntax
      ([type (datum->syntax #'* (filter-scheme-type 'string-char))])
      #''type)))
...
(define-syntax any-set?
  (syntax-rules ()
    ((_ mask x) (not (fx= (fxlogand mask x) 0)))))

(define-syntax all-set?
  (syntax-rules ()
    ((_ mask x) (let ((m mask)) (fx= (fxlogand m x) m)))))

(define-syntax set-flags
  (syntax-rules () ((_ mask x) (fxlogor mask x))))

(define-syntax reset-flags
  (syntax-rules () ((_ mask x) (fxlogand (fxlognot mask) x))))
  ...
  (lambda (exp)
    (syntax-case exp ()
      ((k struct field (flag mask) ...)
       (let ()
         (define getter-name
           (lambda (f) (construct-name #'k #'struct "-" f)))
         (define setter-name
           (lambda (f)
         ...
         (with-syntax ((field-ref (getter-name #'field))
                       ...
                       (f->m (construct-name #'k #'struct "-" #'field "-mask")))
           #'(begin
...
(define-flag-field
  ...
  (was-multiply-referenced
    (sll multiply-referenced prelex-was-flags-offset))
  ; aggregate flags:
...
(macro-define-structure ($c-func)
  ([code-record #f]
   ; (code func free ...)
   [code-object #f]
   ; actual code object created by c-mkcode
   [closure-record #f]
   ; (closure . func), if constant
   [closure #f]))
; actual closure created by c-mkcode, if constant

(define-syntax negated-flonum?
  (syntax-rules () ((_ x) (fx= ($flonum-sign x) 1))))

(define-syntax $nan?
  (syntax-rules ()
    ((_ e) (let ((x e)) (float-type-case [(ieee) (not (fl= x x))])))))
...
(define-syntax exceptional-flonum?
  (syntax-rules ()
    ((_ x) (float-type-case [(ieee) (fx= ($flonum-exponent x) #x7ff)]))))
    ...
    (syntax-rules ()
      ...
      [(_ var expr)
       (begin
         ...
         (define-syntax var
           (identifier-syntax (id (tmp)) ((set! id val) (tmp val)))))])
                      ...
                      (unless (let mem ((ls #'(formal ...)))
                                (and (not (null? ls))
                                     (or (free-identifier=? x (car ls))
                                         (mem (cdr ls)))))
                              (syntax-error x
                                (format "undeclared ~s monad binding" 'name))))
                    #'(x (... ...)))
                  #'(let ((x e) (... ...))
                      (proc formal ... arg (... ...)))))
               ((key proc) #'(key proc ())))))))))
...
(define-syntax make-binding
  (syntax-rules () ((_ type value) (cons type value))))
(define-syntax binding-type
  (syntax-rules () ((_ b) (car b))))
(define-syntax binding-value
  (syntax-rules () ((_ b) (cdr b))))
(define-syntax set-binding-type!
  (syntax-rules () ((_ b v) (set-car! b v))))
(define-syntax set-binding-value!
  (syntax-rules () ((_ b v) (set-cdr! b v))))
...
(constant-case address-bits
  [(32)
   (constant-case segment-table-levels
     [(1) (define-constant segment-t1-bits 19)]
     ; table size: .5M words = 2M bytes
     [(2)
      (define-constant segment-t2-bits 9)
      ; outer-table size: .5k words = 2k bytes
      (define-constant segment-t1-bits 10)])
   ; inner-table size: 1k words = 4k bytes
   (define-constant segment-offset-bits 13)
   ; segment size: 8k bytes (2k ptrs)
   (define-constant card-offset-bits 8)]
  ; card size: 256 bytes (64 ptrs)
  [(64)
   (constant-case segment-table-levels
     [(2)
      (define-constant segment-t2-bits 25)
      ; outer-table size: 32M words = 268M bytes
      (define-constant segment-t1-bits 25)]
     ; inner-table size: 32M words = 268M bytes
     [(3)
      (define-constant segment-t3-bits 17)
      ; outer-table size: 128k words = 1M bytes
      (define-constant segment-t2-bits 17)
      ; middle-table size: 128k words = 1M bytes
      (define-constant segment-t1-bits 16)])
   ; inner-table size: 64k words = 512k bytes
   (define-constant segment-offset-bits 14)
   ; segment size: 16k bytes (2k ptrs)
   (define-constant card-offset-bits 9)])
; card size: 512 bytes (64 ptrs)

(define-constant bytes-per-segment (ash 1 (constant segment-offset-bits)))
(define-constant segment-card-offset-bits
  (- (constant segment-offset-bits) (constant card-offset-bits)))
;;; cards-per-segment must be a multiple of ptr-bits, since gc sometimes
;;; processes dirty bytes in iptr-sized pieces
(define-constant cards-per-segment
  (ash 1 (constant segment-card-offset-bits)))
...
(define-syntax winder-critical?
  (syntax-rules () [(_ w) (vector-ref w 0)]))
(define-syntax winder-in
  (syntax-rules () [(_ w) (vector-ref w 1)]))
(define-syntax winder-out
  (syntax-rules () [(_ w) (vector-ref w 2)]))
    ...
    (define state-case-test
      (lambda (cvar k)
        (with-syntax ((cvar cvar))
          (syntax-case k (-)
            (char (char? (datum char)) #'(char=? cvar char))
            ((char1 - char2)
             (and (char? (datum char1))
                  (char? (datum char2)))
             #'(char<=? char1 cvar char2))
            (predicate (identifier? #'predicate) #'(predicate cvar))))))
    (define state-case-help
      (lambda (cvar clauses)
        (syntax-case clauses (else)
          (((else exp1 exp2 ...)) #'(begin exp1 exp2 ...))
          ((((k ...) exp1 exp2 ...) . more)
           (with-syntax
             (((test ...)
...
(define-syntax set-who!
  (lambda (x)
    (syntax-case x ()
      ...
      [(k id e)
       (identifier? #'id)
       (with-implicit (k who) #'(set! id (let ([who 'id]) e)))])))

(define-syntax define-who
  (lambda (x)
    (syntax-case x ()
      [(k (id . args) b1 b2 ...) #'(k id (lambda args b1 b2 ...))]
      [(k #(prefix id) e)
       (and (identifier? #'prefix) (identifier? #'id))
       (with-implicit (k who)
         (with-syntax ([ext-id (construct-name #'id #'prefix #'id)])
      ...
      [(k id e)
       (identifier? #'id)
       (with-implicit (k who) #'(define id (let ([who 'id]) e)))])))
...
(define-syntax trace-define-who
  (lambda (x)
    (syntax-case x ()
      ...
      [(k id e)
       (identifier? #'id)
       (with-implicit (k who) #'(trace-define id (let ([who 'id]) e)))])))
             ...
             (let ([sym (syntax->datum x)])
               (if (gensym? sym)
                   (cons (datum->syntax x
                         ...
                         x)
                   (cons x
                         (datum->syntax x (gensym (symbol->string sym))))))))
         (with-syntax ([(dtname . dtuid) (split-name #'dtname)]
                       [((vname . vuid) ...)
                        (map split-name #'(vname ...))]
                       [(dtfield ...)
                        (map (lambda (spec)
           ...
           (with-syntax ([dtname? (construct-name #'dtname #'dtname "?")]
                         [dtname-case
                           (construct-name #'dtname #'dtname "-case")]
                         [dtname-variant
                           (construct-name #'dtname #'dtname "-variant")]
                         [(dtname-dtfield ...)
                          (map (lambda (field)
                         ...
                         [((vname-field ...) ...)
                          (map (lambda (vname fields)
                                 (map (lambda (field)
                                        (construct-name #'dtname vname "-" field))
                                      fields))
                               #'(vname ...)
                               #'((field ...) ...))]
                         [(raw-make-vname ...)
                          (map (lambda (x)
                                 (construct-name #'dtname "make-" x))
                               #'(vname ...))]
                         [(make-vname ...)
                          (map (lambda (x)
                                 (construct-name #'dtname #'dtname "-" x))
                               #'(vname ...))]
                         ...
                         [((fvar ...) ...)
                          (map generate-temporaries
                               #'((field ...) ...))])
             #'(module (dtname? (dtname-case dtname-variant vname-field ... ...)
                                dtname-dtfield
                                ...
                                dtname-dtfield-set!
                                ...
  ...
  (syntax-rules ()
    [(_ (name make-name pred?)
     ...
     (module (name make-name pred? accessor ... mutator ...)
       ...
       (module new
         (name make-name pred? accessor ... mutator ...)
         new-defn)
       ...
       (define accessor
         (lambda (x)
           ((if ((let () (import old) pred?) x)
                ...
                (let () (import new) accessor)) x)))
       ...
       (define mutator
         (lambda (x v)
           ((if ((let () (import old) pred?) x)
                (let () (import old) mutator)
                (let () (import new) mutator)) x v)))
       ...)]))
...
(eval-when (load eval)
  ...
  (let ()
        ...
        (syntax-case x ()
          ((_ (name closure?
           ...
           (with-syntax ([(index-base ...) (enumerate (datum (name ...)))])
             ...
             #`(begin
                 (define-constant library-entry-vector-size
    ...
    (declare-library-entries (main #f 0 #f #f)
      ;; fake entry for main, never called directly (part of fasl load)
      (car #f 1 #t #t)
      (cdr #f 1 #t #t)
      (unbox #f 1 #t #t)
      (set-box! #f 2 #t #t)
