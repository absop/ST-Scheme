...
(let ()
  ...
  (define-syntax xvalues
    (lambda (x)
      (syntax-case x ()
        ((key arg ...)
         (with-implicit (key fp tb it) #'(values fp tb it arg ...))))))
  ...
  (define-syntax state-lambda
    (lambda (x)
      (syntax-case x ()
        ((key (arg ...) b1 b2 ...)
         (with-implicit (key xlambda) #'(xlambda (arg ...) b1 b2 ...))))))

  (define-syntax define-state
    (lambda (x)
      (syntax-case x ()
        ((key (name arg ...) b1 b2 ...)
         (with-implicit (key xdefine) #'(xdefine (name arg ...) b1 b2 ...))))))

  (define-syntax *state
    ; move to state
    (lambda (x)
      (syntax-case x ()
        ((key s arg ...) (with-implicit (key xcall) #'(xcall s arg ...))))))

  (define-syntax state-return
    (lambda (x)
      (syntax-case x ()
        ((key type value)
         (with-implicit (key xvalues bfp) #'(xvalues bfp 'type value))))))
    ...
    (lambda (x)
      (syntax-case x ()
        ((key (type value) expr1 expr2 ...)
         ...
         (with-implicit (key xcall xmvlet bfp)
           #'(xmvlet ((bfp type value) (xcall rd-token)) expr1 expr2 ...))))))
      ...
      (syntax-case x ()
        ((key i c b1 b2 ...)
         (with-implicit (key tb)
           #'(let ((g i) (t c))
               (let ((tb (if (fx= g (string-length tb))
                             (string-stretch tb)
                             tb)))
                 (string-set! tb g t)
                 b1
      ...
      (syntax-case x ()
        ((key b1 b2 ...)
         (with-implicit (key it)
           #'(let ((it (or it (make-eq-hashtable))))
               b1
               b2
               ...))))))
  ...
  (define char-name-table
    (make-hashtable char->integer char=?))
      ...
      (case-lambda
        ...
        [(ip tb n m slashed1? slashed2?)
         (if (fold-case? ip slashed1?)
             ...
             (if (fold-case? ip slashed2?)
                 (let ([s2 (string-append (substring tb 0 n)
                             (string-foldcase (substring tb n m)))])
  ...
  (define string-stretch
    (lambda (old)
      ...
      (define overhead-bytes
        (+ (constant header-size-string) 1))
      ; allocator will align anyway, so start with as many as we
      ; can without grabbing more real space
      (define smart-size
        (lambda (n)
          (fx- (c-alloc-align
                 (fx+ overhead-bytes (fx* n (constant string-char-bytes))))
               overhead-bytes)))
    ...
    (lambda (stx)
      (syntax-case stx ()
        [(_ sfd-expr bfp-expr efp-expr)
         #'(let ([sfd sfd-expr] [bfp bfp-expr] [efp efp-expr])
             (if ($current-mso)
      ...
      (cond
        ...
        [(rcb-sfd rcb)
         ($lexical-error (rcb-who rcb)
           ...
           args
           ip
           ($make-source-object (rcb-sfd rcb) bfp fp)
           start?
           ir?)]
        [else
         ($lexical-error (rcb-who rcb)
      ...
      (syntax-case x ()
        [(k str)
         (with-implicit (k rcb xcall)
           #'(when ($port-flags-set? (rcb-ip rcb)
    ...
    (let ((bfp fp))
      (with-read-char c
        (state-case c
          ...
          [((#\0 - #\9))
           (with-stretch-buffer 0 c (*state rd-token-number-or-symbol 1))]
          ...
          [else
           (*state rd-token-symbol c 0 #f rd-token-intern-nonstandard)]))))
    ...
    (with-peek-char c
      (state-case c
        ...
        [(#\{ #\} #\' #\` #\,)
         ...
         (state-return atomic '-)]
        [else
         (with-stretch-buffer 0 #\- (*state rd-token-number-or-symbol 1))])))
  ...
  (define-state (rd-token-dot-dot-dot)
    (with-read-char c
      (state-case c
        ...
        [char-whitespace? (with-unread-char c (state-return atomic '...))]
        [(#\{ #\} #\' #\` #\,)
         (nonstandard-delimiter c)
         (with-unread-char c (state-return atomic '...))]
        [else
    ...
    (with-read-char c
      (state-case c
        ...
        [#\; (if (eq? (rcb-who rcb) 'read-token)
                 ...
                 (xmvlet (() (xcall rd-expression-comment))
                         (*state rd-token)))]
      ...
      (state-case c
        ...
        [#\| (with-peek-char c
               (state-case c
                 ...
                 [else
                  (*state rd-token-block-comment depth)]))]
        [#\# (with-peek-char c
               (state-case c
                 [eof (xcall rd-eof-error "block comment")]
                 [#\| (with-read-char c
        ...
        [((#\0 - #\9))
         (with-stretch-buffer i
           ...
           (*state rd-token-hash-num
                   ...
                   (fx+ i 1)))]
        [#\( (nonstandard "#<n>(...) vector") (state-return vnparen n)]
        ; ) for paren bouncer
        [(#\r #\R)
         (nonstandard "#<n>r number prefix")
         (with-stretch-buffer i c (*state rd-token-number (fx+ i 1)))]
  ...
  (define-state (rd-token-mark n)
    (with-insert-table (state-return mark n)))
        ...
        [(#\x)
         (with-peek-char c1
           (state-case c1
             ...
             [((#\0 - #\9) (#\a - #\f) (#\A - #\F))
              (with-stretch-buffer 0 #\x (*state rd-token-char-hex 0 1))]
    ...
    (with-read-char c
      (state-case c
        ...
        [(#\{ #\} #\' #\` #\,)
         ...
         (with-unread-char c (state-return atomic (int->char n)))]
        [else
         (*state rd-token-to-delimiter i c rd-token-charname)])))
  ...
  (define-state (rd-token-charname n c)
    ...
    (define (r6rs-char-name x)
      (cond [(assq x r6rs-char-names) => cdr] [else #f]))
    (with-unread-char c
      (state-return atomic
                       ...
                       (valid-prefix? s
                         (map symbol->string
                              (if ($port-flags-set?
                                  ...
                                  (let-values
                                    ([(keys vals) (hashtable-entries char-name-table)])
  ...
  (define-state (rd-token-hash-bang2 i undelimited*)
    (cond
      [(ormap (lambda (a) (and (fx= (string-length (car a)) i) a))
              undelimited*)
       =>
       (lambda (a)
         (let ([ip (rcb-ip rcb)])
           (case (cdr a)
             ...
             [(no-fold-case)
              ...
              (*state rd-token)]
             [(chezscheme)
              ($reset-port-flags! ip (constant port-flag-r6rs))
              (*state rd-token)]
             [else
              (xcall rd-error #f #t "unexpected #!~s" (car a))])))]
      [else
       (with-read-char c
         (state-case c
           ...
           [else
            (let ([undelimited*
                    (filter (lambda (a) (char=? (string-ref (car a) i) c))
                            undelimited*)])
              (if (null? undelimited*)
        ...
        (state-case c
          ...
          [#\\ (with-read-char c
                 (state-case c
                   ...
                   [(#\newline #\return #\nel #\ls)
                    (*state rd-token-string-whitespace i c)]
                   ...
                   [((#\0 - #\7))
                    ...
                    (with-read-char c1
                      (state-case c1
                        [eof (with-unread-char c1
                               (xcall rd-eof-error "string"))]
                        [((#\0 - #\7))
                         (with-read-char c2
                           (state-case c2
                             [eof (with-unread-char c2
  ...
  (define-state (rd-token-string-whitespace i c)
    (state-case c
      [eof (with-unread-char c (xcall rd-eof-error "string"))]
      [(#\newline #\nel #\ls) (*state rd-token-string-skipwhite i)]
      [(#\return)
       (with-peek-char c
         (state-case c
           [eof (xcall rd-eof-error "string")]
  ...
  (xdefine (rd-token-string-hex-char i n char-bfp)
    (with-read-char c1
      (state-case c1
        ...
        [((#\0 - #\9) (#\a - #\f) (#\A - #\F))
         (*state rd-token-string-hex-char
                 i
  ...
  (xdefine (rd-make-number-or-symbol n)
    (let ([z ($str->num tb
               ...
               10
               #f
               ($port-flags-set? (rcb-ip rcb) (constant port-flag-r6rs)))])
      (cond
        [(number? z) z]
        [(eq? z 'norep)
         (xcall rd-error #t #t "cannot represent ~a" (substring tb 0 n))]
        [(eq? z '!r6rs)
         (xcall rd-nonstandard-error
                (format "~a number" (substring tb 0 n)))]
        [else
  ...
  (xdefine (rd-make-number n)
    (let ([z ($str->num tb
      ...
      (cond
        [(number? z) z]
        [(and (eq? z #f) (with-peek-char c (eof-object? c)))
    ...
    (state-case c
      ...
      [#\\ (with-read-char c
             (state-case c
               ...
               [#\x (*state rd-token-symbol-hex-char
                            i
                            0
                            (and fp (fx- fp 2))
                            slashed?
      ...
      [#\| (nonstandard "|...| symbol escape")
           (*state rd-token-symbol-bar i next)]
      [(#\( #\[ #\" #\; #\#)
       ;)] for paren bouncer
       (with-unread-char c (*state next i slashed?))]
  ...
  (xdefine (rd-top-level type value)
    (if (case type [(eof) #t] [(atomic) (eof-object? value)] [else #f])
        (values value fp)
  ...
  (xdefine (rd-fix-graph x)
    (define rd-set-car!
      (lambda (obj idx val) (set-car! obj val)))
    (define rd-set-cdr!
      (lambda (obj idx val) (set-cdr! obj val)))
    (define rd-set-box!
      (lambda (obj idx val) (set-box! obj val)))
    (define rd-field-set!
      (lambda (obj i val)
      ...
      (lambda (dr update! obj idx)
        (delayed-record-update-set! dr
          (let ((f (delayed-record-update dr)))
            (lambda (val) (update! obj idx val) (f val))))))
           ...
           (do ([fields (csv7:record-type-field-names d) (cdr fields)] [i 0 (+ i 1)])
               ...
               (when (csv7:record-field-accessible? d i)
                     (rd-fix-graph
                       ...
                       x
                       i))))]
        [(box? x) (rd-fix-graph (unbox x) rd-set-box! x #f)]))
    (let ((p (cons x #f)))
  ...
  (xdefine (GROSS-HACK type value) (xcall rd-help type value))
  ...
  (xdefine (rd-help type value)
    (case type
      ...
      [(vfxnparen)
       (xmvlet ((v) (xcall rd-sized-fxvector value)) (xvalues v v))]
      [(vu8paren) (xmvlet ((v) (xcall rd-bytevector bfp 0)) (xvalues v v))]
      [(vu8nparen)
       (xmvlet ((v) (xcall rd-sized-bytevector value)) (xvalues v v))]
      [(box) (xcall rd-box)]
      [(fasl)
       (xcall rd-error
         #f
      ...
      [else
       (xcall rd-error #f #f "unexpected internal token type ~s" type)]))
      ...
      (case type
        ...
        [(dot)
         (with-token (type value)
           (case type
             [(rparen)
              (xcall rd-error #f #f "expected one item after dot (.)")]
             [(eof) (let ([bfp expr-bfp]) (xcall rd-eof-error "list"))]
             [else
              (xmvlet ((x stripped-x) (xcall rd type value))
                (with-token (type value)
    ...
    (let ([expr-bfp bfp])
      (with-token (type value)
        (case type
          [(eof)
           (let ([bfp expr-bfp])
             (xcall rd-eof-error (symbol->string kind)))]
          [else
           (xmvlet ((x stripped-x) (xcall rd type value))
                   (xvalues (list kind x)
                     (and (rcb-a? rcb)
                          (list kind stripped-x))))]))))
             ...
             [(or (record-reader (symbol->string name))
              ...
              (lambda (rtd)
                (let ((decls (csv7:record-type-field-decls rtd)))
                  (xmvlet ((vals stripped-vals)
                           (xcall rd-record-tail
                                  expr-bfp
                                  (length decls)
                                  name))
                    ...
                    (let ([vals (if stripped-vals
                      ...
                      (let loop ((fds decls) (vs (or stripped-vals vals)))
                        (if (null? fds)
                            (xvalues (apply (record-constructor rtd) vals)
                                     (and (rcb-a? rcb)
                                          (apply (record-constructor rtd) stripped-vals)))
                            (if (and (apply (lambda (m t n)
                                ...
                                (loop (cdr fds) (cdr vs)))))))))]
             [else
              (xcall rd-error #f #t "unrecognized record name ~s" name)])]))))
  ...
  (xdefine (rd-vector expr-bfp i)
    (with-token (type value)
      (case type
        [(rparen)
         (xvalues (make-vector i) (and (rcb-a? rcb) (make-vector i)))]
        ...
        [(rparen)
         (when (fx< 0 i n)
               (let ((prev (vector-ref v (fx- i 1))))
                 (do ([i i (fx+ i 1)]) ((fx= i n)) (vector-set! v i prev)))
               (when stripped-v
    ...
    (let ([expr-bfp bfp])
      (with-token (type value)
        (case type
          [(eof)
           (let ([bfp expr-bfp])
             (xcall rd-eof-error "s-expression comment"))]
          [else
           (xmvlet ((x stripped-x) (xcall rd type value)) (xvalues))]))))
      ...
      (define read-token
        (lambda (ip sfd)
          ...
          (let ([fp (and (port-has-port-position? ip)
                         ($port-flags-set? ip
                           (constant port-flag-char-positions))
                         (port-position ip))])
            (let ([rcb (make-rcb ip sfd #f who)] [tb ""] [bfp fp] [it #f])
              (with-token (type value) (values type value bfp fp))))))
      (case-lambda
        ...
        (let ([fp (or fp
                      (and (port-has-port-position? ip)
                           ($port-flags-set? ip
                             (constant port-flag-char-positions))
                           (port-position ip)))])
          (let ([rcb (make-rcb ip sfd (and a? sfd fp #t) who)]
                [tb ""]
                [bfp fp]
                [it #f])
            (call-with-token rd-top-level)))))
    ...
    (lambda (sfd)
      ...
      (let ([name (source-file-descriptor-name sfd)])
        (or (and ($fixed-path? name) (source-port name))
            (let ([dir* (append (source-directories)
                                (map car
                                     (library-directories)))])
  ...
  (let ([source-lines-cache (make-weak-eq-hashtable)])
      ...
      (lambda (sfd fp use-cache?)
        (define (binary-search table name)
          (let loop ([lo 0] [hi (vector-length table)])
            (if (fx= (fx+ 1 lo) hi)
                (values name hi (fx+ 1 (fx- fp (vector-ref table lo))))
    ...
    (set! $clear-source-lines-cache
      ...
      (lambda () (hashtable-clear! source-lines-cache))))
  ...
  (set! $source-file-descriptor
    (let ()
                  ...
                  (loop (fx+ i 1)
                        (fxlogxor (fxsrl crc 8)
                          (vector-ref crc16-table
                            (fxlogand (fxlogxor crc (bytevector-u8-ref s i))
                                      #xff)))))))))
      (define go/reset
        (lambda (ifn ip)
      ...
      (case-lambda
        ...
        [(ifn ip reset?) (if reset? (go/reset ifn ip) (go ifn ip))])))
      ...
      (case-lambda
        ...
        [(x c)
         (unless (and (symbol? x) (valid-name? (symbol->string x)))
                 ($oops 'char-name "~s is not a valid character name" x))
         (with-tc-mutex
           ...
           (cond
             ...
             (else
              ($oops 'char-name "~s is not a character" c))))]))))
;let
  ...
  (case-lambda
    [(name/rtd)
     (cond
       ...
       [(symbol? name/rtd) ($sgetprop name/rtd 'record-reader #f)]
       [else
        ($oops 'record-reader "invalid input ~s" name/rtd)])]
    [(name/rtd rtd/false)
     (cond
       [(not rtd/false)
        (cond
        ...
        (let ([rtd rtd/false])
          (cond
            ...
            [else
             ($oops 'record-reader "invalid first argument ~s" name/rtd)]))]
       [else
        ($oops 'record-reader "invalid second argument ~s" rtd/false)])]))
...
(begin
  ...
  (char-name 'newline #\newline)
  ; must come after linefeed entry
