...
(define pretty-one-line-limit
  ($make-thread-parameter 60
    (lambda (x)
      (unless (and (fixnum? x) (fx> x 0))
              ($oops 'pretty-one-line-limit
                     "~s is not a positive fixnum"
                     x))
      x)))
...
(let ()
      ...
      (syntax-case fmt (quote var read-macro meta alt bracket s)
        ...
        [(read-macro str sym)
         (syntax-case expr ($primitive)
           ...
           [else s])]
        [(meta) (syntax-case expr (meta) [(meta . x) 0.0] [else s])]
        [(alt . fmt*)
         (let f ([fmt* (cdr #'fmt*)]
                 [min-s (score-fmt (car #'fmt*) expr s)])
  ...
  (define (select-alt-fmt fmt expr)
    (cond
      [(syntax-case fmt (alt) [(alt . fmt*) #'fmt*] [_ #f])
       =>
       (lambda (fmt*)
         (let ([fmt (car fmt*)])
           (let f ([fmt* (cdr fmt*)]
                   [min-s (score-fmt fmt expr)]
                   [min-fmt fmt])
             (if (or (null? fmt*) (fl= min-s 0.0))
  ...
  (define-syntax limit? (syntax-rules () ((_ x) (eq? x 0))))
  ...
  (define mk-prty-vector
    (lambda (vlen vref x lev len)
      (if (limit? lev)
          (make-prty '() 5 "(...)")
          (let ([prtys (let ([m (if (print-vector-length)
                         ...
                         (if (and len (fx<= len m))
                             (mk-prty-vector-help
                               ...
                               (fx- len 1)
                               (list (make-prty '() 3 "...")))
                             (mk-prty-vector-help vref x (decr lev) len m '())))])
  ...
  (define mk-prty-list-help
    (lambda (x lev len fmt-tail)
      (define (fmt-next fmt-tail)
        (syntax-case fmt-tail (fill)
          [() (values 'x '())]
          [(fill tab fmt dots) (values #'fmt fmt-tail)]
          [(tab fmt dots)
           (and (tab? #'tab) (dots? #'dots))
           (values #'fmt fmt-tail)]
          [(fmt tab dots)
           (and (tab? #'tab) (dots? #'dots))
           (values #'fmt fmt-tail)]
          [(tab fmt . fmt-tail) (tab? #'tab) (values #'fmt #'fmt-tail)]
          [(fmt dots) (dots? #'dots) (values #'fmt fmt-tail)]
          [(fmt . fmt-tail) (values #'fmt #'fmt-tail)]))
  ...
  (define pretty-fill
    (lambda (obj start pps oneline tab space?)
      ...
      (when (> (apply + (map prty-len obj)) (* 2 room))
            (set! room
              (min (- (pretty-line-length) start) (pretty-one-line-limit))))
      (let f ([obj obj] [space? space?])
        (let* ([p (car obj)] [len (prty-len p)])
          (cond
            ...
            [space? (pretty-tab (+ tab start) #f) (f obj #f)]
            [else
    ...
    (lambda (n oneline)
      (if oneline
          ...
          (begin
            ...
            (set! col n)
            (set! room
              (min (- (pretty-line-length) n) (pretty-one-line-limit)))))))
    ...
    (lambda (in out)
      ...
      (let ([i ($open-file-input-port
            ...
            [o ($open-file-output-port
                 ...
                 (buffer-mode block)
                 (current-transcoder))])
        (on-reset (begin (close-input-port i) (delete-file out #f))
          (on-reset (close-output-port o)
            (let loop ()
              (let ([x (read i)])
                (unless (eof-object? x)
                        (pretty-print x o)
  ...
  (pretty-format 'cond
    '(_ #f (alt [bracket test '=> 0 exp] [bracket test 0 exp ...]) ...))
  ...
  (pretty-format 'extend-syntax
    '(_ (keys 0 ...) #f [bracket e1 0 e2 ...] ...))
  ...
  (pretty-format 'meta '(meta))
  ; meta is handled as a special case
  (pretty-format 'meta-cond
    '(_ #f (alt [bracket test '=> 0 exp] [bracket test 0 exp ...]) ...))
  ...
  (pretty-format 'syntax-case
    '(_ exp (fill 0 lit ...) #f [bracket pat 0 e ...] ...))
  (pretty-format 'syntax-rules
    '(_ (fill 0 lit ...) #f [bracket pat 0 e ...] ...))
  (pretty-format 'rec '(_ id #f e))
  (pretty-format 'record-case
    '(_ exp #f [bracket tag (x 0 ...) 0 exp ...] ...))
  (pretty-format 'set! '(_ id #f e ...))
  (pretty-format 'trace-case-lambda
    '(_ name #f [bracket (fill 0 x ...) 0 e ...] ...))
  (pretty-format 'trace-define '(_ (fill 0 x ...) #f e ...))
  (pretty-format 'trace-define-syntax '(_ (x ...) #f e ...))
  (pretty-format 'trace-do
    '(_ ([bracket x ...] 0 ...) 3 (e1 0 ...) #f e ...))
